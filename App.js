...
function App() {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      // We do NOT call signInAnonymously here.
      // We only listen for existing auth state.
      unsubscribeAuth = authInstance.onAuthStateChanged(async (currentUser) => {
        setUser(currentUser);
        setIsAuthReady(true); // Auth state has been determined

        // If a user is logged in AND they are NOT anonymous, redirect.
        if (currentUser && !currentUser.isAnonymous) {
            console.log("Používateľ je prihlásený (nie anonymný), presmerovanie na logged-in-my-data.html");
            window.location.href = 'logged-in-my-data.html';
            return; // Stop further rendering for this component
        }
        // If no user, or user is anonymous, stay on index.html.
    ...
    const fetchSettings = async () => {
      ...
      try {
          ...
          const unsubscribeSettings = settingsDocRef.onSnapshot(docSnapshot => {
            ...
            // setPageLoading(false); // This is now handled by auth state check
          }, error => {
  ...
  // We now wait for both auth and settings to be ready.
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        // This block will only render if user is null or anonymous
        React.createElement(
          React.Fragment,
          null,
          isRegistrationOpen ? (
