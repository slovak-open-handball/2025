    ...
    const GOOGLE_APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbx7oRQ3JN8fh6b1-07Aew2pAhY8uK7K7dxxfywwtzv6opOGcYcBfnUYc4XL6MRvTYKExw/exec"; // <--- AKTUALIZOVANÁ URL
    ...
    function UsersManagementApp() {
      ...
      const handleDeleteUser = async () => {
        ...
        try {
          ...
          console.log(`Používateľ ${userToDelete.email} bol zmazaný z databázy.`);
          
          // 2. Odoslať požiadavku na Apps Script na zmazanie používateľa z Firebase Authentication
          try {
              const payload = {
                  action: 'deleteUserFromAuth',
                  uid: userToDelete.uid,
                  email: userToDelete.email // Pre logovanie v Apps Scripte
              };
              console.log("Odosielam požiadavku na Apps Script na zmazanie používateľa z Authentication:", JSON.stringify(payload, null, 2));
              const response = await fetch(GOOGLE_APPS_SCRIPT_URL, {
                  method: 'POST',
                  mode: 'no-cors', // Dôležité pre cross-origin požiadavky na Apps Script
                  headers: {
                      'Content-Type': 'application/json',
                  },
                  body: JSON.stringify(payload)
              });
              console.log("Požiadavka na zmazanie z Authentication bola odoslaná (no-cors režim).");
              ...
              console.error("UsersManagementApp: Chyba pri odosielaní požiadavky na Apps Script na zmazanie z Authentication:", appsScriptError);
              setUserNotificationMessage(`Používateľ ${userToDelete.email} bol zmazaný z databázy, ale nastala chyba pri zmazaní z Authentication: ${appsScriptError.message}`);
          }

          // 3. Uložiť notifikáciu pre všetkých administrátorov
          await db.collection('artifacts').doc(appId).collection('public').doc('data').collection('adminNotifications').add({
            message: `Používateľ ${userToDelete.email} bol zmazaný.`,
            timestamp: firebase.firestore.FieldValue.serverTimestamp(),
            recipientId: 'all_admins',
            read: false
          });
          ...
          closeConfirmationModal(); // Zavrieť potvrdzovací modal
                        ...
                        React.createElement(
                            ...
                            users.map((u) => (
                                React.createElement(
                                    ...
                                    React.createElement('td', { className: 'py-3 px-6 text-left' }, `${u.firstName || ''} ${u.lastName || ''}`),
