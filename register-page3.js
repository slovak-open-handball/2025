...
export function Page3Form({
    ...
    const unsubscribeUsers = onSnapshot(usersCollectionRef, (querySnapshot) => {
      ...
      querySnapshot.forEach((userDoc) => {
        ...
        Object.entries(teams).forEach(([categoryName, teamsArray]) => {
          ...
          if (teamCount > 0) {
            // Predpokladáme, že categoryName je rovnaký ako categoryId
            // Ak nie, musíš tu doplniť mapovanie názov → ID
  ...
  // Existuje aspoň jedna kategória s voľnou kapacitou?
  ...
  const getAvailableCategoryOptions = (currentIndex = -1) => {
    ...
    return allCategoryIds
      .map(catId => {
        ...
        return {
          id: catId,
          name: name + (isFull ? " (plná)" : ""), // voliteľne pridáme (plná) do názvu
  ...
  const nextButtonClasses = `
    ...
    ${loading || !isRecaptchaReady || !isFormValidPage3 || Object.keys(categoriesData).length === 0 || !hasAtLeastOneFreeCategory
    ...
    React.createElement(
      ...
      !isCategoriesLoaded ? (
        ...
        React.createElement(
          ...
          // Ak je aspoň jedna voľná kategória → zobrazíme riadky
            ...
            React.createElement(
              ...
              selectedCategoryRows.map((row, index) => (
                React.createElement(
                  ...
                  // Červený text pod selectom iba ak je vybraná plná kategória
