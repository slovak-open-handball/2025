// teams.js - KOMPLETNE OPRAVENÁ VERZIA
...
const TeamsOverviewApp = (props) => {
    ...
    useEffect(() => {
        ...
        console.log('[Stats Effect] isLoadingMembers:', isLoadingMembers);
        
        // AK PREBIEHA NAČÍTANIE ČLENOV, POČKÁME
        if (isLoadingMembers) {
            console.log('[Stats Effect] ⏳ Načítavam členov tímu, čakám...');
            return;
        }
        
        const teamKey = `${rosterTeamName || selectedTeamDetails?.teamName || ''}_${rosterCategoryName || selectedTeamDetails?.category || ''}`;
        
        // AK NIE JE ŽIADNY TÍM, VYMAŽEME ŠTATISTIKY
        if (!rosterTeamName && !selectedTeamDetails?.teamName) {
            console.log('[Stats Effect] Žiadny vybraný tím, vymazávam štatistiky');
            setMembersStats({});
            setCurrentTeamKey('');
            return;
        }
        
        // AK NIE SÚ ČLENOVIA, POČKÁME
        if (!teamRoster || teamRoster.length === 0) {
            console.log('[Stats Effect] Čakám na načítanie členov tímu...');
            return;
        }
        ...
        if (!currentTeamName || !currentCategoryName) {
            console.log('[Stats Effect] Chýba teamName alebo categoryName');
            setMembersStats({});
            return;
        }
        ...
        // AK SME UŽ NASTAVILI KĽÚČ A JE ROVNAKÝ, NEROBÍME NIČ
        if (currentTeamKey === teamKey) {
            console.log('[Stats Effect] Tím je už načítaný, preskakujem');
            return;
        }
    ...
    }, [teamRoster, rosterTeamName, rosterCategoryName, selectedTeamDetails, updateTrigger, isLoadingMembers]);
    ...
    const loadTeamRoster = (teamName, categoryName) => {
        ...
        // NASTAVÍME, ŽE PREBIEHA NAČÍTANIE
        ...
        const handleMembersUpdate = (members) => {
            ...
            // AK UŽ SME DOSTALI ČLENOV, IGNORUJEME DUPLICITY
            if (membersReceived) {
                console.log('[loadTeamRoster] ⚠️ Duplicitné volanie handleMembersUpdate, ignorujem');
                return;
            }
    ...
    useEffect(() => {
        ...
        if (selectedTeamDetails && selectedTeamDetails.occurrences && selectedTeamDetails.occurrences.length > 0 && hasCategoryInUrl) {
            ...
            if (selectedOcc) {
                ...
                // VOLÁME LEN AK SA ZMENIL NÁZOV TÍMU ALEBO KATEGÓRIA A NEPREBIEHA NAČÍTANIE
                const currentTeamName = rosterTeamName || '';
                const currentCategoryName = rosterCategoryName || '';
                if ((selectedOcc.teamName !== currentTeamName || categoryName !== currentCategoryName) && !isLoadingMembers) {
