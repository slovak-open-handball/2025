...
//            console.log('='.repeat(60));
    ...
    if (selectedMatch) {
        ...
        // Stav pre plávajúce okno
const [showFloatingMatchBar, setShowFloatingMatchBar] = useState(false);
const floatingBarRef = useRef(null);
const mainContentRef = useRef(null);

// Listener na scrollovanie
useEffect(() => {
    const handleScroll = () => {
        if (!mainContentRef.current) return;
        
        // Získame pozíciu hlavného obsahu
        const mainContentRect = mainContentRef.current.getBoundingClientRect();
        const mainContentTop = mainContentRect.top;
        
        // Ak sme preskrolovali hlavičku, zobrazíme plávajúce okno
        if (mainContentTop < 0) {
            setShowFloatingMatchBar(true);
        } else {
            setShowFloatingMatchBar(false);
        }
    };
    ...
    // Komponent pre plávajúce okno
    const FloatingMatchBar = () => {
        if (!showFloatingMatchBar) return null;
        
        // Získame aktuálne skóre a čas
        const currentHomeScore = matchScore.home;
        const currentAwayScore = matchScore.away;
        const currentTime = formatMatchTime(cleanPlayingTime || 0);
        const currentPeriod = selectedMatch.currentPeriod || 1;
        const category = categories.find(c => c.name === selectedMatch.categoryName);
        const maxPeriods = category?.periods || 2;
        ...
        switch (selectedMatch.status) {
            ...
            case 'paused':
                ...
                statusText = 'Pozastavené';
                break;
            default:
                statusColor = 'text-gray-600 bg-gray-50';
                statusText = 'Naplánované';
        }
        ...
        return React.createElement(
            'div',
            {
                className: 'fixed top-0 left-0 right-0 bg-white shadow-lg border-b border-gray-200 z-[200] animate-slideDown',
                style: {
            ...
            React.createElement(
                ...
                React.createElement(
                    ...
                    // Ľavá strana - domáci tím a skóre
                    React.createElement(
                        'div',
                    ...
                    React.createElement(
                        ...
                        (selectedMatch.status === 'in-progress' || selectedMatch.status === 'paused') && React.createElement(
                            ...
                            React.createElement(
                                ...
                                `${currentPeriod}/${maxPeriods} perióda`
                            )
                        ),
                        React.createElement(
                            'div',
                    ...
                    // Pravá strana - hosťovský tím a tlačidlo pre skok hore
                    React.createElement(
                        'div',
                        ...
                        React.createElement(
                            ...
                            React.createElement('i', { className: 'fa-solid fa-arrow-up text-gray-600' })
                        )
                    )
                )
            )
    ...
    const mainContent = React.createElement(
        ...
        return React.createElement(
            React.Fragment,
            null,
            React.createElement(FloatingMatchBar, null),
            mainContent,
