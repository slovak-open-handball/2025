...
const matchesHallApp = ({ userProfileData }) => {
    ...
    // OPRAVENÝ useEffect PRE MAPOVANIE NÁZVOV TÍMOV
    useEffect(() => {
        if (!selectedMatch) return;
        ...
        const mapTeamNames = async () => {
            ...
            // Získame zobrazovacie názvy (ak existujú)
            ...
            let homeResolvedName = homeTeamDisplayName || selectedMatch.homeTeamIdentifier;
            let awayResolvedName = awayTeamDisplayName || selectedMatch.awayTeamIdentifier;
            
            try {
                // 🔥 DOMÁCI - vždy skúsime získať mapovaný názov (aj keď už máme displayName)
                if (homeTeamDisplayName && homeTeamDisplayName !== selectedMatch.homeTeamIdentifier) {
                    ...
                    // Fallback: použijeme identifikátor
                ...
                // 🔥 HOSŤOVSKÍ - vždy skúsime získať mapovaný názov
            ...
            console.log(`🔍 [MAPOVANIE] Výsledok pre súpisky:`);
            ...
            // 🔥 DÔLEŽITÉ: VŽDY nastavíme resolved names (aj keď už máme displayName)
            setHomeTeamResolvedName(homeResolvedName);
            setAwayTeamResolvedName(awayResolvedName);
            
            // Aktualizujeme displayName v selectedMatch len ak neexistujú
            setSelectedMatch(prev => {
        ...
        mapTeamNames();
    }, [selectedMatch?.id, selectedMatch?.homeTeamIdentifier, selectedMatch?.awayTeamIdentifier]); // Odstránime podmienku na homeDisplayName
...
//                console.error("Chyba pri načítaní názvu haly:", error);
    ...
    useEffect(() => {
        ...
        // 🔥 2. MANUÁLNY VÝSLEDOK (finalScore) pre ukončený zápas - NASTAVÍME SKÓRE A NENAČÍTAME UDALOSTI
        if (selectedMatch.finalScore && selectedMatch.status === 'completed' && !selectedMatch.forfeitResult) {
            // NASTAVÍME SPRÁVNE SKÓRE Z finalScore
            setMatchScore({ 
            ...
            const unsubscribe = onSnapshot(q, (snapshot) => {
                ...
                // 🔥 DÔLEŽITÉ: Ak máme udalosti, NEPREPÍŠEME matchScore z udalostí!
                // Zachováme manuálne zadaný výsledok z finalScore
                // (tento komentár je dôležitý)
            }, (error) => {
                console.error("Chyba pri načítaní udalostí zápasu:", error);
            });
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            // (len ak nemá finalScore alebo forfeitResult)
            if (selectedMatch.status !== 'completed' || (!selectedMatch.finalScore && !selectedMatch.forfeitResult)) {
                setMatchScore({ home: homeScore, away: awayScore });
            } else if (selectedMatch.finalScore && !selectedMatch.forfeitResult) {
                // Ak má finalScore, zachováme ho (toto je už ošetrené v bode 2 vyššie)
                // Tento fallback je pre prípad, že by sa udalosti načítali skôr ako finalScore
                setMatchScore({ 
                    home: selectedMatch.finalScore.home, 
                    away: selectedMatch.finalScore.away 
                });
