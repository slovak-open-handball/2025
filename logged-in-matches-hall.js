...
//            console.log('='.repeat(60));
    ...
    const selectMatch = async (match) => {
        ...
        // ✅ VÝPIS NÁZOV TÍMOV Z DOM A KATEGÓRIE DO KONZOLY
        console.log('');
        ...
        setTimeout(() => {
            ...
            // Ak sme nezískali názvy z DOM, použijeme identifikátory ako fallback
            const finalHomeTeamName = homeTeamNameFromDOM || match.homeTeamIdentifier;
            const finalAwayTeamName = awayTeamNameFromDOM || match.awayTeamIdentifier;
            const categoryNameDisplay = match.categoryName || 'neurčená';
            
            console.log(`🏠 DOMÁCI TÍM: ${finalHomeTeamName}`);
            console.log(`✈️ HOSŤOVSKÝ TÍM: ${finalAwayTeamName}`);
...
const renderFullTeamToUI = (teamDetails, teamType) => {
    ...
    // Nájdeme správny kontajner podľa typu tímu
    ...
    if (!targetContainer) {
        console.log(`   ⚠️ Nenašiel sa kontajner pre ${teamType === 'home' ? 'domáci' : 'hosťovský'} tím`);
        return false;
    }
    ...
    // Vykreslíme hráčov
    renderPlayersToUI(teamDetails, teamType, targetContainer);
    
    // Vykreslíme realizačný tím
    renderStaffToUI(teamDetails, teamType, targetContainer);
    
    console.log(`   ✅ ${teamType === 'home' ? 'Domáci' : 'Hosťovský'} tím bol úspešne vykreslený`);
    return true;
};
...
window.setBothTeamsDetails = async (homeTeamName, homeCategory, awayTeamName, awayCategory) => {
    ...
    if (homeTeamName && homeCategory) {
        await window.setHomeTeamDetails(homeTeamName, homeCategory);
    }
    if (awayTeamName && awayCategory) {
        await window.setAwayTeamDetails(awayTeamName, awayCategory);
    }
};
