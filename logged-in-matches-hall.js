...
// Funkcia na výpis členov tímu podľa názvu tímu a kategórie
function printTeamMembers(teamName, categoryName) {
    console.log(`\n🔍 Hľadám tím: "${teamName}" v kategórii: "${categoryName}"\n`);
    
    // Skontrolujeme, či sú dostupné dáta
    if (!window.users || window.users.length === 0) {
        console.log('❌ Dáta používateľov nie sú dostupné. Skúste neskôr.');
        return;
    }
    ...
    for (const user of window.users) {
        ...
        for (const [category, teams] of Object.entries(user.teams)) {
            ...
            for (const team of teams) {
                // Kontrola názvu tímu (môže byť čiastočná zhoda)
                const matchTeamName = team.teamName && (
                    team.teamName.toLowerCase().includes(teamName.toLowerCase()) ||
                    teamName.toLowerCase().includes(team.teamName.toLowerCase())
                );
                
                if (matchTeamName || !teamName) {
                    foundTeams.push({
                        userEmail: user.email,
                        userName: user.displayName || user.email,
                        category: category,
                        teamName: team.teamName || 'Neznámy názov',
                        groupName: team.groupName,
                        order: team.order,
                        players: team.playerDetails || [],
                        menStaff: team.menTeamMemberDetails || [],
                        womenStaff: team.womenTeamMemberDetails || []
                    });
                }
            }
        }
    ...
    if (foundTeams.length === 0) {
        console.log(`❌ Nebol nájdený žiadny tím s názvom "${teamName}"${categoryName ? ` v kategórii "${categoryName}"` : ''}`);
        return;
    }
    
    // Výpis nájdených tímov
    console.log(`✅ Nájdených ${foundTeams.length} tímov:\n`);
    
    foundTeams.forEach((team, index) => {
        console.log(`📌 TÍM #${index + 1}: ${team.teamName}`);
        console.log(`   🏷️ Kategória: ${team.category}`);
        console.log(`   👤 Používateľ: ${team.userName} (${team.userEmail})`);
        console.log(`   📊 Skupina: ${team.groupName}, Poradie: ${team.order}`);
        console.log(`   ─────────────────────────────`);
        
        // Hráči
        console.log(`   🧑‍🤝‍🧑 HRÁČI (${team.players.length}):`);
        if (team.players.length === 0) {
            console.log(`      Žiadni hráči`);
        } else {
            team.players.forEach((player, idx) => {
                console.log(`      ${(idx + 1).toString().padStart(2)}. ${player.lastName || ''} ${player.firstName || ''} ${player.jerseyNumber ? `(#${player.jerseyNumber})` : ''}`);
            });
        }
        
        // Realizačný tím - muži
        console.log(`   👨‍🏫 REALIZAČNÝ TÍM - MUŽI (${team.menStaff.length}):`);
        if (team.menStaff.length === 0) {
            console.log(`      Žiadni členovia`);
        } else {
            team.menStaff.forEach((member, idx) => {
                console.log(`      ${(idx + 1).toString().padStart(2)}. ${member.lastName || ''} ${member.firstName || ''}`);
            });
        }
        
        // Realizačný tím - ženy
        console.log(`   👩‍🏫 REALIZAČNÝ TÍM - ŽENY (${team.womenStaff.length}):`);
        if (team.womenStaff.length === 0) {
            console.log(`      Žiadne členky`);
        } else {
            team.womenStaff.forEach((member, idx) => {
                console.log(`      ${(idx + 1).toString().padStart(2)}. ${member.lastName || ''} ${member.firstName || ''}`);
            });
        }
