...
const matchesHallApp = ({ userProfileData }) => {
    ...
    useEffect(() => {
        ...
        // Počkáme 500ms a potom znova načítame aktuálne dáta zápasu
        const timer = setTimeout(async () => {
            try {
                ...
                if (matchSnap.exists()) {
                    const freshMatchData = matchSnap.data();
                    
                    // Skontrolujeme, či sa niečo zmenilo (finalScore, forfeitResult, status)
                    let hasChanges = false;
                    
                    if (JSON.stringify(freshMatchData.finalScore) !== JSON.stringify(selectedMatch.finalScore)) {
                        hasChanges = true;
                    }
                    if (JSON.stringify(freshMatchData.forfeitResult) !== JSON.stringify(selectedMatch.forfeitResult)) {
                        hasChanges = true;
                    }
                    if (freshMatchData.status !== selectedMatch.status) {
                        hasChanges = true;
                    }
                    
                    if (hasChanges) {
                        console.log('🔄 Aktualizujem selectedMatch s čerstvými dátami z databázy');
                ...
                console.error('Chyba pri synchronizácii selectedMatch:', error);
            }
        }, 500);
    ...
    useEffect(() => {
        const loadTeamDetails = async () => {
            ...
            console.log(`   Kategória zápasu: "${selectedMatch.categoryName}"`);
            
            // 🔥 NAČÍTANIE DOMÁCEHO TÍMU
            if (homeTeamResolvedName && homeTeamResolvedName !== selectedMatch.homeTeamIdentifier) {
                console.log(`🔍 [SÚPISKY] Hľadám domáci tím "${homeTeamResolvedName}" v kategórii "${selectedMatch.categoryName}"`);
                ...
                if (homeDetailsLoaded && homeDetailsLoaded.team) {
                    ...
                    console.log(`   📊 Počet hráčov: ${homeDetailsLoaded.team.playerDetails?.length || 0}`);
                    setHomeTeamData(homeDetailsLoaded);
                } else {
                    console.log(`❌ [SÚPISKY] Domáci tím "${homeTeamResolvedName}" nebol nájdený!`);
                    setHomeTeamData(null);
                }
            } else {
                console.log(`⚠️ [SÚPISKY] Domáci tím je rovnaký ako identifikátor, preskakujem`);
                setHomeTeamData(null);
            }
            
            // 🔥 NAČÍTANIE HOSŤOVSKÉHO TÍMU
            if (awayTeamResolvedName && awayTeamResolvedName !== selectedMatch.awayTeamIdentifier) {
                console.log(`🔍 [SÚPISKY] Hľadám hosťovský tím "${awayTeamResolvedName}" v kategórii "${selectedMatch.categoryName}"`);
                ...
                if (awayDetailsLoaded && awayDetailsLoaded.team) {
                    ...
                    console.log(`   📊 Počet hráčov: ${awayDetailsLoaded.team.playerDetails?.length || 0}`);
                    setAwayTeamData(awayDetailsLoaded);
                } else {
                    console.log(`❌ [SÚPISKY] Hosťovský tím "${awayTeamResolvedName}" nebol nájdený!`);
                    setAwayTeamData(null);
                }
            } else {
                console.log(`⚠️ [SÚPISKY] Hosťovský tím je rovnaký ako identifikátor, preskakujem`);
                setAwayTeamData(null);
            }
        ...
        loadTeamDetails();
    }, [homeTeamResolvedName, awayTeamResolvedName, selectedMatch?.id, selectedMatch?.categoryName, users.length]); // 🔥 PRIDANÉ users.length ako závislosť
    ...
    // OPRAVENÝ useEffect PRE MAPOVANIE NÁZVOV TÍMOV - používa správne názvy
    useEffect(() => {
        ...
        if (selectedMatch.homeDisplayName && selectedMatch.awayDisplayName) {
            console.log(`✅ [MAPOVANIE] Už máme zmapované názvy: home="${selectedMatch.homeDisplayName}", away="${selectedMatch.awayDisplayName}"`);
            return;
        }
        ...
        const mapTeamNames = async () => {
            ...
            console.log(`🔍 [MAPOVANIE] Začínam mapovať názvy tímov...`);
            console.log(`   Pôvodný domáci identifikátor: "${selectedMatch.homeTeamIdentifier}"`);
            console.log(`   Pôvodný hosťovský identifikátor: "${selectedMatch.awayTeamIdentifier}"`);
            
            // 🔥 KRITICKÉ: Najprv získame správne NÁZVY TÍMOV (nie identifikátory)
            // Tieto názvy sú zobrazené v zozname zápasov (napr. "U12 D 3A")
            let homeTeamDisplayName = selectedMatch.homeDisplayName;
            let awayTeamDisplayName = selectedMatch.awayDisplayName;
            
            // Ak nemáme homeDisplayName, skúsime ho získať
            if (!homeTeamDisplayName) {
                homeTeamDisplayName = getTeamNameByIdentifier(selectedMatch.homeTeamIdentifier);
                console.log(`   🔄 Získaný názov domáceho tímu: "${homeTeamDisplayName}"`);
            }
            
            // Ak nemáme awayDisplayName, skúsime ho získať
            if (!awayTeamDisplayName) {
                awayTeamDisplayName = getTeamNameByIdentifier(selectedMatch.awayTeamIdentifier);
                console.log(`   🔄 Získaný názov hosťovského tímu: "${awayTeamDisplayName}"`);
            }
            
            let homeResolvedName = homeTeamDisplayName;
            let awayResolvedName = awayTeamDisplayName;
            
            try {
                // 🔥 DOMÁCI - najprv použijeme NÁZOV TÍMU (nie identifikátor)
                if (homeTeamDisplayName && homeTeamDisplayName !== selectedMatch.homeTeamIdentifier) {
                    console.log(`   🔄 Domáci - používam názov: "${homeTeamDisplayName}"`);
                    
                    // Prvá konverzia: názov tímu -> mapovaný názov
                    let homeFirst = await window.matchTracker.getTeamNameByDisplayId(homeTeamDisplayName);
                    if (homeFirst && homeFirst !== homeTeamDisplayName) {
                        console.log(`   🔄 Domáci - prvá konverzia: "${homeTeamDisplayName}" -> "${homeFirst}"`);
                        
                        // Druhá konverzia (ak treba)
                        let homeSecond = await window.matchTracker.getTeamNameByDisplayId(homeFirst);
                        if (homeSecond && homeSecond !== homeFirst) {
                            console.log(`   🔄 Domáci - druhá konverzia: "${homeFirst}" -> "${homeSecond}"`);
                            homeResolvedName = homeSecond;
                        } else {
                            homeResolvedName = homeFirst;
                        }
                    } else if (homeFirst) {
                        homeResolvedName = homeFirst;
                    }
                } else {
                    // Fallback: použijeme pôvodný identifikátor
                    console.log(`   ⚠️ Domáci - nemám názov tímu, používam identifikátor: "${selectedMatch.homeTeamIdentifier}"`);
                    let homeFirst = await window.matchTracker.getTeamNameByDisplayId(selectedMatch.homeTeamIdentifier);
                    if (homeFirst && homeFirst !== selectedMatch.homeTeamIdentifier) {
                        console.log(`   🔄 Domáci - konverzia identifikátora: "${selectedMatch.homeTeamIdentifier}" -> "${homeFirst}"`);
                        homeResolvedName = homeFirst;
                    }
                }
                
                // 🔥 HOSŤOVSKÍ - najprv použijeme NÁZOV TÍMU (nie identifikátor)
                if (awayTeamDisplayName && awayTeamDisplayName !== selectedMatch.awayTeamIdentifier) {
                    console.log(`   🔄 Hosťovskí - používam názov: "${awayTeamDisplayName}"`);
                    
                    // Prvá konverzia: názov tímu -> mapovaný názov
                    let awayFirst = await window.matchTracker.getTeamNameByDisplayId(awayTeamDisplayName);
                    if (awayFirst && awayFirst !== awayTeamDisplayName) {
                        console.log(`   🔄 Hosťovskí - prvá konverzia: "${awayTeamDisplayName}" -> "${awayFirst}"`);
                        
                        // Druhá konverzia (ak treba)
                        let awaySecond = await window.matchTracker.getTeamNameByDisplayId(awayFirst);
                        if (awaySecond && awaySecond !== awayFirst) {
                            console.log(`   🔄 Hosťovskí - druhá konverzia: "${awayFirst}" -> "${awaySecond}"`);
                            awayResolvedName = awaySecond;
                        } else {
                            awayResolvedName = awayFirst;
                        }
                    } else if (awayFirst) {
                        awayResolvedName = awayFirst;
                    }
                    ...
                    // Fallback: použijeme pôvodný identifikátor
                    console.log(`   ⚠️ Hosťovskí - nemám názov tímu, používam identifikátor: "${selectedMatch.awayTeamIdentifier}"`);
                    let awayFirst = await window.matchTracker.getTeamNameByDisplayId(selectedMatch.awayTeamIdentifier);
                    if (awayFirst && awayFirst !== selectedMatch.awayTeamIdentifier) {
                        console.log(`   🔄 Hosťovskí - konverzia identifikátora: "${selectedMatch.awayTeamIdentifier}" -> "${awayFirst}"`);
                        awayResolvedName = awayFirst;
                    }
                }
            } catch (error) {
            ...
            console.log(`🔍 [MAPOVANIE] Výsledok:`);
            console.log(`   Domáci: "${homeTeamDisplayName || selectedMatch.homeTeamIdentifier}" -> "${homeResolvedName}"`);
            console.log(`   Hosťovskí: "${awayTeamDisplayName || selectedMatch.awayTeamIdentifier}" -> "${awayResolvedName}"`);
            ...
            setSelectedMatch(prev => {
                ...
                return {
                    ...prev,
                    homeDisplayName: homeTeamDisplayName || homeResolvedName,
                    awayDisplayName: awayTeamDisplayName || awayResolvedName
                };
            });
        };
        
        mapTeamNames();
    }, [selectedMatch?.id, selectedMatch?.homeTeamIdentifier, selectedMatch?.awayTeamIdentifier]);
    
    // ============================================================================
    // NAČÍTANIE SUSPENDOVANÝCH HRÁČOV ZA MODRÚ KARTU PRE DOMÁCICH (OPRAVENÉ S ONESKORENÍM)
    // ============================================================================
    
    // PRE DOMÁCICH - UPRAVENÝ useEffect
    useEffect(() => {
        // 🔥 POČKÁME NA NAČÍTANIE homeTeamData
        if (!selectedMatch || (selectedMatch.status !== 'in-progress' && selectedMatch.status !== 'paused')) {
            setIsLoadingSuspensionsHome(false);
            return;
        }
        
        // 🔥 KRITICKÉ: Počkáme, kým sa načítajú dáta tímu (súpisky)
        if (!homeTeamData || !homeTeamData.team || !homeTeamData.team.playerDetails) {
            console.log('⏳ [DOMÁCI] Čakám na načítanie súpisky domáceho tímu...');
            // Nenastavujeme isLoading na false, len čakáme
            return;
        }
        ...
        const loadSuspensions = async () => {
            ...
            // 🔥 2. ONESKORENIE 2 sekundy
            console.log('⏳ [DOMÁCI] Čakám 2 sekundy...');
            await new Promise(resolve => setTimeout(resolve, 2000));
            
            // 🔥 3. POČKÁME NA USERS A SUPERSTRUCTURE TEAMS
            ...
            // 🔥 POUŽIJEME homeTeamData namiesto teamDetails?.team
            const teamData = homeTeamData?.team;
            ...
            console.log(`📊 [DOMÁCI] Nájdených zápasov: ${teamMatches.length}`);
            ...
            // Pre každého hráča v tíme (použijeme homeTeamData)
            for (let i = 0; i < teamData.playerDetails.length; i++) {
                ...
                if (player.removedForMatch === selectedMatch.id) {
                    console.log(`   ⏭️ [DOMÁCI] Hráč ${player.lastName} ${player.firstName} je odstránený pre tento zápas`);
                    continue;
                }
                ...
                if (blueCardEvents.length > 0) {
                    ...
                    if (isSuspended) {
                        newSuspendedPlayers[i] = {
                            player: player,
                            reason: `Vylúčený na ${blueCardSuspensionMatches} zápas(ov) za modrú kartu`,
            ...
            setIsLoadingSuspensionsHome(false);
            console.log(`✅ [DOMÁCI] Nájdených suspendovaných: ${Object.keys(newSuspendedPlayers).length}`);
        };
        ...
        return () => clearTimeout(timer);
    }, [selectedMatch?.homeTeamIdentifier, selectedMatch?.id, selectedMatch?.categoryName, users, superstructureTeams, homeTeamData]);

    // ============================================================================
    // NAČÍTANIE SUSPENDOVANÝCH HRÁČOV ZA MODRÚ KARTU PRE HOSŤOVSKÝCH (OPRAVENÉ)
    // ============================================================================
    
    useEffect(() => {
        // 🔥 POČKÁME NA NAČÍTANIE awayTeamData
        if (!selectedMatch || (selectedMatch.status !== 'in-progress' && selectedMatch.status !== 'paused')) {
        ...
        // 🔥 KRITICKÉ: Počkáme, kým sa načítajú dáta tímu (súpisky)
        if (!awayTeamData || !awayTeamData.team || !awayTeamData.team.playerDetails) {
            console.log('⏳ [HOSŤOVSKÍ] Čakám na načítanie súpisky hosťovského tímu...');
            // Nenastavujeme isLoading na false, len čakáme
            return;
        }
        ...
        const loadSuspensions = async () => {
            ...
            // 🔥 2. ONESKORENIE 2 sekundy
            console.log('⏳ [HOSŤOVSKÍ] Čakám 2 sekundy...');
            await new Promise(resolve => setTimeout(resolve, 2000));
            ...
            // 🔥 POUŽIJEME awayTeamData namiesto teamDetails?.team
            const teamData = awayTeamData?.team;
            ...
            // 🔥 POUŽIJEME KONVERTOVANÝ NÁZOV TÍMU NA VYHĽADÁVANIE ZÁPASOV
            const teamMatches = await getTeamMatchesByNameAndCategory(actualTeamName, currentCategoryName);
            console.log(`📊 [HOSŤOVSKÍ] Nájdených zápasov: ${teamMatches.length}`);
            ...
            // Pre každého hráča v tíme (použijeme awayTeamData)
            for (let i = 0; i < teamData.playerDetails.length; i++) {
                const player = teamData.playerDetails[i];
                if (!player) continue;
                
                if (player.removedForMatch === selectedMatch.id) {
                    console.log(`   ⏭️ [HOSŤOVSKÍ] Hráč ${player.lastName} ${player.firstName} je odstránený pre tento zápas`);
                    continue;
                }
            ...
            setIsLoadingSuspensionsAway(false);
            console.log(`✅ [HOSŤOVSKÍ] Nájdených suspendovaných: ${Object.keys(newSuspendedPlayers).length}`);
        };
        ...
        return () => clearTimeout(timer);
    }, [selectedMatch?.awayTeamIdentifier, selectedMatch?.id, selectedMatch?.categoryName, users, superstructureTeams, awayTeamData]);
    ...
    // NOVÝ useEffect PRE AKTUALIZÁCIU NÁZVOV TÍMOV PRI NÁVRATE NA ZOZNAM - S DVOJITOU KONVERZIOU
    useEffect(() => {
        ...
        if (selectedMatch === null && matches.length > 0 && teamManagerReady) {
            console.log('🔄 Návrat na zoznam - aktualizujem názvy tímov (s dvojitou konverziou)...');
            ...
            const updateTeamNamesForAllMatches = async () => {
                ...
                for (let i = 0; i < updatedMatches.length; i++) {
                    ...
                    // Použijeme matchTracker na získanie zmapovaných názvov S DVOJITOU KONVERZIOU
                    if (window.matchTracker && typeof window.matchTracker.getTeamNameByDisplayId === 'function') {
                        try {
                            // 🔥 DOMÁCI - DVOJITÁ KONVERZIA
                            let homeFirst = window.matchTracker.getTeamNameByDisplayId(homeIdentifier);
                            if (homeFirst && typeof homeFirst.then === 'function') homeFirst = await homeFirst;
                            
                            if (homeFirst && homeFirst !== homeIdentifier) {
                                let homeSecond = window.matchTracker.getTeamNameByDisplayId(homeFirst);
                                if (homeSecond && typeof homeSecond.then === 'function') homeSecond = await homeSecond;
                                
                                if (homeSecond && homeSecond !== homeFirst) {
                                    homeDisplayName = homeSecond;
                                    console.log(`   🔄 [DOMÁCI] Dvojitá konverzia: "${homeIdentifier}" -> "${homeFirst}" -> "${homeDisplayName}"`);
                                } else {
                                    homeDisplayName = homeFirst;
                                    console.log(`   🔄 [DOMÁCI] Jedna konverzia: "${homeIdentifier}" -> "${homeDisplayName}"`);
                                }
                            }
                            
                            // 🔥 HOSŤOVSKÍ - DVOJITÁ KONVERZIA
                            let awayFirst = window.matchTracker.getTeamNameByDisplayId(awayIdentifier);
                            if (awayFirst && typeof awayFirst.then === 'function') awayFirst = await awayFirst;
                            
                            if (awayFirst && awayFirst !== awayIdentifier) {
                                let awaySecond = window.matchTracker.getTeamNameByDisplayId(awayFirst);
                                if (awaySecond && typeof awaySecond.then === 'function') awaySecond = await awaySecond;
                                
                                if (awaySecond && awaySecond !== awayFirst) {
                                    awayDisplayName = awaySecond;
                                    console.log(`   🔄 [HOSŤOVSKÍ] Dvojitá konverzia: "${awayIdentifier}" -> "${awayFirst}" -> "${awayDisplayName}"`);
                                } else {
                                    awayDisplayName = awayFirst;
                                    console.log(`   🔄 [HOSŤOVSKÍ] Jedna konverzia: "${awayIdentifier}" -> "${awayDisplayName}"`);
                                }
                            }
                        } catch (e) {
                            console.warn(`Chyba pri získavaní názvu: ${e}`);
                            homeDisplayName = homeIdentifier;
                            awayDisplayName = awayIdentifier;
                        }
                    } else if (window.teamManager && typeof window.teamManager.getTeamNameByDisplayIdSync === 'function') {
                        try {
                            // Fallback na teamManager (len ak matchTracker nie je dostupný)
                            homeDisplayName = window.teamManager.getTeamNameByDisplayIdSync(homeIdentifier) || homeIdentifier;
                            awayDisplayName = window.teamManager.getTeamNameByDisplayIdSync(awayIdentifier) || awayIdentifier;
                        } catch (e) {
                    ...
                    const newHomeDisplay = homeDisplayName || homeIdentifier || 'Neznámy tím';
                    const newAwayDisplay = awayDisplayName || awayIdentifier || 'Neznámy tím';
                    
                    if (currentHomeDisplay !== newHomeDisplay || currentAwayDisplay !== newAwayDisplay) {
                        updatedMatches[i] = {
                            ...match,
                            homeDisplayName: newHomeDisplay,
                            awayDisplayName: newAwayDisplay
                        };
                        hasChanges = true;
                ...
                if (hasChanges) {
                    setMatches(updatedMatches);
                    console.log('✅ Zoznam zápasov bol aktualizovaný pri návrate na zoznam (s dvojitou konverziou)');
                }
            };
...
//                console.error("Chyba pri načítaní názvu haly:", error);
    ...
    // ============================================================================
    // OPRAVA 2: UPRAVIŤ useEffect PRE NAČÍTANIE UDALOSTÍ ZÁPASU - PODPORA MANUÁLNEHO VÝSLEDKU A KONTUMÁCIE
    // ============================================================================
    
    useEffect(() => {
        if (!selectedMatch || !window.db) return;
    
        // 🔥 1. KONTUMÁCIA - použijeme forfeitResult
        ...
        // 🔥 2. MANUÁLNY VÝSLEDOK (finalScore) pre ukončený zápas
        ...
        // 🔥 3. NORMÁLNY PRÍPAD - načítame udalosti a vypočítame skóre
        const eventsRef = collection(window.db, 'matchEvents');
        const q = query(eventsRef, where("matchId", "==", selectedMatch.id));
    ...
    // V useEffect pre načítanie zápasov (matches), pridajte nastavenie homeDisplayName a awayDisplayName
    useEffect(() => {
        if (!window.db) return;
        ...
        if (!currentHallId || typeof currentHallId !== 'string') {
            console.log('⏳ HallId nie je k dispozícii...');
            setLoading(false);
            return;
        }
        ...
        const unsubscribe = onSnapshot(q, async (snapshot) => {
            ...
            for (const doc of snapshot.docs) {
                ...
                // 🔥 Hneď pri načítaní zápasu získame zobrazovacie názvy tímov
                let homeDisplayName = data.homeTeamIdentifier;
                let awayDisplayName = data.awayTeamIdentifier;
                
                // Skúsime získať zobrazovací názov z používateľských dát
                if (users && users.length > 0) {
                    // Pre domáci tím
                    const homeTeamName = getTeamNameByIdentifier(data.homeTeamIdentifier);
                    if (homeTeamName && homeTeamName !== data.homeTeamIdentifier) {
                        homeDisplayName = homeTeamName;
                    }
                    
                    // Pre hosťovský tím
                    const awayTeamName = getTeamNameByIdentifier(data.awayTeamIdentifier);
                    if (awayTeamName && awayTeamName !== data.awayTeamIdentifier) {
                        awayDisplayName = awayTeamName;
                    }
                }
        ...
        return () => unsubscribe();
    }, [users]); // 🔥 PRIDANÉ users ako závislosť, aby sa prepočítali názvy keď sa users načítajú
...
//                    console.log(`🔄 Druhá konverzia: "${firstPassResult}" -> "${secondPassResult}"`);
    ...
    // OPRAVENÁ FUNKCIA getTeamDetailsByDisplayName
    const getTeamDetailsByDisplayName = async (teamDisplayName, categoryName) => {
        ...
        console.log(`   📊 Users k dispozícii: ${users.length}`);
        ...
        console.log(`   🎯 Vyriešený názov tímu: "${resolvedTeamName}"`);
        
        // 2. VYHĽADÁVAME V POUŽÍVATEĽSKÝCH DÁTACH (users)
        if (users && users.length > 0) {
            console.log(`   🔍 Prehľadávam ${users.length} používateľov...`);
            ...
            for (const user of users) {
                ...
                const userTeams = user.teams[categoryName];
                if (!userTeams || !Array.isArray(userTeams)) {
                    console.log(`   ⏭️ Používateľ ${user.email} nemá tímy v kategórii "${categoryName}"`);
                    continue;
                }
                
                console.log(`   📋 Používateľ ${user.email} má ${userTeams.length} tím(ov) v kategórii "${categoryName}":`);
                userTeams.forEach((t, idx) => {
                    console.log(`      ${idx + 1}. "${t.teamName}" (skupina: ${t.groupName}, poradie: ${t.order})`);
                });
                
                // Hľadáme podľa vyriešeného názvu (ak existuje) alebo podľa pôvodného displayName
                const team = userTeams.find(t => 
                    t.teamName === resolvedTeamName || 
                    t.teamName === teamDisplayName
                );
                ...
                if (team) {
                    ...
                    // Detailný výpis prvých 3 hráčov pre debug
                    if (team.playerDetails && team.playerDetails.length > 0) {
                        console.log(`   🧑‍🤝‍🧑 Prví hráči:`);
                        team.playerDetails.slice(0, 3).forEach((p, idx) => {
                            console.log(`      ${idx + 1}. ${p.lastName} ${p.firstName} (#${p.jerseyNumber})`);
                        });
            ...
            console.log(`   ⚠️ Žiadni používatelia nie sú načítaní!`);
        }
        
        // 3. FALLBACK: Skúsime vyhľadať v superstructureTeams (ak existuje)
        if (superstructureTeams && Object.keys(superstructureTeams).length > 0) {
            console.log(`   🔍 Skúšam superstructureTeams...`);
        ...
        console.log(`   💡 Tip: Skontrolujte, či v používateľských dátach existuje tím s názvom "${resolvedTeamName}" v kategórii "${categoryName}"`);
        return null;
    };
...
//        return null;
    ...
    const showAllMatches = async () => {
        ...
        // Ak hallId v stave nie je, skúsime získať z userProfileData (priamo z props)
        if (!currentHallId && userProfileData?.hallId) {
            currentHallId = userProfileData.hallId;
        }
        
        // Ak stále nemáme, skúsime z globálnej premennej (fallback)
        if (!currentHallId && window.globalUserProfileData?.hallId) {
            currentHallId = window.globalUserProfileData.hallId;
        }
            ...
            if (!teamData || !teamData.team) {
                return React.createElement('div', { className: 'mt-4 p-4 text-center' },
                    React.createElement('div', { className: 'flex items-center justify-center gap-2 text-gray-500' },
                        ...
                        React.createElement('span', null, 'Načítavam súpisku...')
                    )
                );
            }
                            ...
                            React.createElement(
                                ...
                                !showPlayerStats ? React.createElement(
                                    ...
                                    React.createElement(
                                        ...
                                        renderPlayersSection(homeTeamData, 'home', homeTeamName, suspendedPlayersHome, isLoadingSuspensionsHome, homeTeamNameReady)
                                    ),
                                    ...
                                    React.createElement(
                                        ...
                                        renderPlayersSection(awayTeamData, 'away', awayTeamName, suspendedPlayersAway, isLoadingSuspensionsAway, awayTeamNameReady)
                                    ),
