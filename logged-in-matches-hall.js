...
// ============================================================================
// POMOCNÁ FUNKCIA NA ZÍSKANIE NÁZVU TÍMU CEZ matchTracker
// ============================================================================

/**
 * Získa názov tímu pomocou window.matchTracker.getTeamNameByDisplayId()
 * @param {string} identifier - Identifikátor tímu (napr. "U12 D 2B")
 * @returns {Promise<string>} - Názov tímu alebo pôvodný identifikátor
 */
const getTeamNameFromTracker = async (identifier) => {
    if (!identifier) return 'Neznámy tím';
    
    // Počkáme na matchTracker
    let waitCount = 0;
    while (!window.matchTracker && waitCount < 50) {
    ...
    if (window.matchTracker && typeof window.matchTracker.getTeamNameByDisplayId === 'function') {
        try {
            ...
            // Ak je výsledok Promise, počkáme naň
            const teamName = (result && typeof result.then === 'function') ? await result : result;
            if (teamName && teamName !== identifier) {
                return teamName;
            }
        } catch (error) {
            console.warn(`Chyba pri mapovaní názvu tímu "${identifier}":`, error);
        }
    }
...
const getTeamMatchesByNameAndCategory = async (teamDisplayName, categoryName) => {
    ...
    try {
        ...
        for (const doc of allMatchesSnap.docs) {
            ...
            // 🔥 2. Získame MAPOVANÉ NÁZVY pre oba tímy v zápase cez matchTracker
            const homeMappedName = await getTeamNameFromTracker(match.homeTeamIdentifier);
            const awayMappedName = await getTeamNameFromTracker(match.awayTeamIdentifier);
...
const getTeamMatchesByName = async (teamName) => {
    ...
    try {
        ...
        for (const doc of allMatchesSnap.docs) {
            ...
            // 🔥 POUŽIJEME matchTracker na mapovanie názvov tímov
            const homeTeamName = await getTeamNameFromTracker(match.homeTeamIdentifier);
            const awayTeamName = await getTeamNameFromTracker(match.awayTeamIdentifier);
...
const getTeamMatchesByDisplayName = async (teamDisplayName, categoryName) => {
    ...
    try {
        ...
        // 🔥 ZÍSKAME ČISTÝ NÁZOV TÍMU cez matchTracker
        const cleanTeamName = await getTeamNameFromTracker(teamDisplayName);
        ...
        for (const doc of allMatchesSnap.docs) {
            ...
            const homeDisplayName = await getTeamNameFromTracker(match.homeTeamIdentifier);
            const awayDisplayName = await getTeamNameFromTracker(match.awayTeamIdentifier);
...
const matchesHallApp = ({ userProfileData }) => {
    ...
    useEffect(() => {
        ...
        const syncTeamNamesWithRoster = async () => {
            ...
            if (homeTeamData && homeTeamData.team && homeTeamData.team.teamName) {
                ...
                const currentDisplayName = await getTeamNameFromTracker(selectedMatch.homeTeamIdentifier);
            ...
            if (awayTeamData && awayTeamData.team && awayTeamData.team.teamName) {
                ...
                const currentDisplayName = await getTeamNameFromTracker(selectedMatch.awayTeamIdentifier);
    ...
    useEffect(() => {
        ...
        const updateTeamNamesInMatches = async () => {
            ...
            for (let i = 0; i < updatedMatches.length; i++) {
                ...
                let homeDisplayName = await getTeamNameFromTracker(homeIdentifier);
                let awayDisplayName = await getTeamNameFromTracker(awayIdentifier);
    ...
    }, [matches.length, teamManagerReady]);
    ...
    useEffect(() => {
        ...
        if (!selectedMatch && matches.length > 0 && teamManagerReady) {
            ...
            const updateAllMatchNames = async () => {
                ...
                for (let i = 0; i < updatedMatches.length; i++) {
                    ...
                    let homeDisplayName = await getTeamNameFromTracker(match.homeTeamIdentifier);
                    let awayDisplayName = await getTeamNameFromTracker(match.awayTeamIdentifier);
        ...
        const mapTeamNames = async () => {
            ...
            let homeResolvedName = await getTeamNameFromTracker(selectedMatch.homeTeamIdentifier);
            let awayResolvedName = await getTeamNameFromTracker(selectedMatch.awayTeamIdentifier);
    ...
    useEffect(() => {
        ...
        const loadSuspensions = async () => {
            ...
            const teamDisplayName = await getTeamNameFromTracker(teamIdentifier);
    ...
    // Tento useEffect zabezpečí, že keď sa zmení users, selectedMatch sa prekreslí
    useEffect(() => {
    ...
    const removePlayerFromRoster = async () => {
        ...
        try {
            ...
            // Uložíme informáciu o odstránení pre konkrétny zápas
            ...
            // 🔥 DÔLEŽITÉ: Aktualizujeme homeTeamData alebo awayTeamData
            if (playerTeam === 'home') {
                setHomeTeamData(prev => {
                    if (!prev) return prev;
                    const updatedTeamData = { ...prev };
                    if (updatedTeamData.team && updatedTeamData.team.playerDetails) {
                        updatedTeamData.team.playerDetails = [...updatedTeamData.team.playerDetails];
                        if (playerIndex !== -1 && updatedTeamData.team.playerDetails[playerIndex]) {
                            updatedTeamData.team.playerDetails[playerIndex] = {
                                ...updatedTeamData.team.playerDetails[playerIndex],
                                removedForMatch: selectedMatch.id
                            };
                        }
                        // Aktualizujeme matchSpecificRemovals
                        if (!updatedTeamData.team.matchSpecificRemovals) {
                            updatedTeamData.team.matchSpecificRemovals = {};
                        }
                        if (!updatedTeamData.team.matchSpecificRemovals[selectedMatch.id]) {
                            updatedTeamData.team.matchSpecificRemovals[selectedMatch.id] = {
                                removedPlayersForMatch: [],
                                removedStaff: []
                            };
                        }
                        updatedTeamData.team.matchSpecificRemovals[selectedMatch.id].removedPlayersForMatch = 
                            [...updatedTeamData.team.matchSpecificRemovals[selectedMatch.id].removedPlayersForMatch, removedPlayer];
                    }
    ...
    const restoreStaffToRoster = async (member, team, teamDetails, staffType) => {
        ...
        try {
            ...
            if (staffIndex !== -1) {
                ...
                // 🔥 DÔLEŽITÉ: Aktualizujeme homeTeamData alebo awayTeamData
                const setTeamData = team === 'home' ? setHomeTeamData : setAwayTeamData;
                setTeamData(prev => {
                    if (!prev) return prev;
                    const updatedTeamData = { ...prev };
                    if (updatedTeamData.team) {
                        const staffArrayName = staffType === 'men' ? 'menTeamMemberDetails' : 'womenTeamMemberDetails';
                        if (updatedTeamData.team[staffArrayName] && staffIndex !== -1) {
                            updatedTeamData.team[staffArrayName] = [...updatedTeamData.team[staffArrayName]];
                            if (updatedTeamData.team[staffArrayName][staffIndex]) {
                                const updatedMember = { ...updatedTeamData.team[staffArrayName][staffIndex] };
                                if (updatedMember.removedForMatch) {
                                    delete updatedMember.removedForMatch[selectedMatch.id];
                                    if (Object.keys(updatedMember.removedForMatch).length === 0) {
                                        delete updatedMember.removedForMatch;
                                    }
                                }
                                updatedTeamData.team[staffArrayName][staffIndex] = updatedMember;
                            }
                        }
                        // Aktualizujeme matchSpecificRemovals
                        if (updatedTeamData.team.matchSpecificRemovals?.[selectedMatch.id]) {
                            const newRemoved = { ...updatedTeamData.team.matchSpecificRemovals[selectedMatch.id] };
                            newRemoved.removedStaff = newRemoved.removedStaff.filter(
                                r => !(r.firstName === member.firstName && r.lastName === member.lastName)
                            );
                            if (newRemoved.removedPlayersForMatch.length === 0 && newRemoved.removedStaff.length === 0) {
                                delete updatedTeamData.team.matchSpecificRemovals[selectedMatch.id];
                            } else {
                                updatedTeamData.team.matchSpecificRemovals[selectedMatch.id] = newRemoved;
                            }
                            if (Object.keys(updatedTeamData.team.matchSpecificRemovals).length === 0) {
                                delete updatedTeamData.team.matchSpecificRemovals;
                            }
                        }
                    }
                    return updatedTeamData;
    ...
    // Funkcia na odstránenie člena RT zo súpisky (OPRAVENÁ - aktualizuje aj homeTeamData/awayTeamData)
    const removeStaffFromRoster = async () => {
        ...
        try {
            ...
            // Nájdeme index člena RT v príslušnom poli podľa vlastností
            ...
            // 2. Skúsime podľa mena a priezviska
            if (staffIndex === -1) {
                staffIndex = staffArray.findIndex(m => 
            ...
            // 🔥 DÔLEŽITÉ: Aktualizujeme homeTeamData alebo awayTeamData
            const setTeamData = staffTeam === 'home' ? setHomeTeamData : setAwayTeamData;
            setTeamData(prev => {
                if (!prev) return prev;
                const updatedTeamData = { ...prev };
                if (updatedTeamData.team) {
                    ...
                    // Ak máme pole staff, vytvoríme jeho kópiu
                    if (updatedTeamData.team[staffArrayName] && staffIndex !== -1) {
                        updatedTeamData.team[staffArrayName] = [...updatedTeamData.team[staffArrayName]];
                        ...
                        if (updatedTeamData.team[staffArrayName][staffIndex]) {
                            const updatedMember = { ...updatedTeamData.team[staffArrayName][staffIndex] };
                            if (!updatedMember.removedForMatch) {
                                updatedMember.removedForMatch = {};
                            }
                            updatedMember.removedForMatch[selectedMatch.id] = true;
                            updatedTeamData.team[staffArrayName][staffIndex] = updatedMember;
                        }
                    }
                    ...
                    updatedTeamData.team.matchSpecificRemovals[selectedMatch.id].removedStaff = 
                        [...updatedTeamData.team.matchSpecificRemovals[selectedMatch.id].removedStaff, removedStaff];
                }
                return updatedTeamData;
            });
    ...
    const restorePlayerToRoster = async (player, team, teamDetails) => {
        ...
        try {
            ...
            // Nájdeme hráča v poli playerDetails
            let playerIndex = -1;
            ...
            // 2. Skúsime podľa mena a čísla dresu
            if (playerIndex === -1) {
                playerIndex = teamData.playerDetails.findIndex(p => 
                    p.firstName === player.firstName && 
                    p.lastName === player.lastName && 
            ...
            if (playerIndex !== -1) {
                ...
                // 🔥 KRITICKÉ: Aktualizujeme lokálny stav users
                setUsers(prevUsers => {
                    return prevUsers.map(user => {
                        if (user.id === teamDetails.userId) {
                            return {
                ...
                if (team === 'home') {
                    setHomeTeamData(prev => {
                        ...
                        const updatedTeamData = { ...prev };
                        if (updatedTeamData.team && updatedTeamData.team.playerDetails) {
                            // Aktualizujeme hráča v poli
                            updatedTeamData.team.playerDetails = [...updatedTeamData.team.playerDetails];
                            if (playerIndex !== -1 && updatedTeamData.team.playerDetails[playerIndex]) {
                                updatedTeamData.team.playerDetails[playerIndex] = {
                                    ...updatedTeamData.team.playerDetails[playerIndex],
                                    removedForMatch: undefined
                                };
                            }
                            // Aktualizujeme matchSpecificRemovals
                            if (updatedTeamData.team.matchSpecificRemovals) {
                                updatedTeamData.team.matchSpecificRemovals = { ...updatedTeamData.team.matchSpecificRemovals };
                                if (updatedTeamData.team.matchSpecificRemovals[selectedMatch.id]) {
                                    const newRemoved = { ...updatedTeamData.team.matchSpecificRemovals[selectedMatch.id] };
                                    newRemoved.removedPlayersForMatch = newRemoved.removedPlayersForMatch.filter(
    ...
    const confirmForfeitMatch = async () => {
        ...
        try {
            ...
            const homeTeamName = await getTeamNameFromTracker(selectedMatch?.homeTeamIdentifier) || 'Domáci';
            const awayTeamName = await getTeamNameFromTracker(selectedMatch?.awayTeamIdentifier) || 'Hostia';
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            const homeTeamName = await getTeamNameFromTracker(selectedMatch.homeTeamIdentifier);
            const awayTeamName = await getTeamNameFromTracker(selectedMatch.awayTeamIdentifier);
            ...
            if (users && users.length > 0) {
                for (const user of users) {
                    if (user.teams && Object.keys(user.teams).length > 0) {
                        // Ticho - pre debug len ak je potrebné
                    }
                }
            }
    ...
    useEffect(() => {
        ...
        const checkDataAndLog = () => {
            ...
            const homeTeamName = getTeamNameFromTracker(selectedMatch.homeTeamIdentifier);
            const awayTeamName = getTeamNameFromTracker(selectedMatch.awayTeamIdentifier);
    ...
    const getTeamNameSafe = async (identifier) => {
        if (!identifier) return 'Neznámy tím';
        return await getTeamNameFromTracker(identifier);
    };

    // FUNKCIA NA ZÍSKANIE NÁZVU TÍMU PODĽA IDENTIFIKÁTORA (POUŽÍVA matchTracker)
    const getTeamNameByIdentifier = getTeamNameFromTracker;

    /**
     * Získa detaily tímu (súpisku) podľa názvu a kategórie z používateľských dát
     */
    const getTeamDetailsByDisplayName = async (teamDisplayName, categoryName) => {
        ...
        // 1. NAJPRV SKÚSIME CEZ matchTracker získať čistý názov tímu
        let resolvedTeamName = await getTeamNameFromTracker(teamDisplayName);
    ...
    if (selectedMatch) {
        ...
        const getTeamDisplayName = (teamSide, identifier) => {
            if (teamSide === 'home') {
                ...
                return getTeamNameFromTracker(identifier);
            } else {
        ...
        const renderPlayersSection = (teamData, teamType, teamName, suspendedPlayers, isLoadingSuspensions, isNameReady) => {
            ...
            // 🔥 OPRAVA: Získame teamData a teamDetails pre správne fungovanie
            ...
            // 🔥 DÔLEŽITÉ: Vytvoríme teamDetails objekt pre funkcie restore a edit
            const teamDetailsForCallbacks = {
                team: team,
                userId: userId,
                userEmail: teamData.userEmail,
                userDisplayName: teamData.userDisplayName
            };
            
            // Získanie dĺžky vylúčenia z kategórie (v sekundách)
            const currentCategory = categories.find(c => c.name === selectedMatch?.categoryName);
            const exclusionDurationSeconds = (currentCategory?.exclusionTime || 2) * 60;
            
            // Funkcia na získanie aktívnych vylúčení
            ...
            // Kontrola vylúčenia za modrú kartu
            const blueCardSuspensionMatches = getBlueCardSuspensionMatches();
            ...
            return React.createElement(
                        ...
                        [...activePlayersWithOriginalIndex]
                            ...
                            .map((player, displayIdx) => {
                                ...
                                if (isMatchCompleted) {
                                    ...
                                    onClickHandler = () => openEditPlayerModal(player, teamType, teamDetailsForCallbacks, false);
                ...
                // Sekcia Ostatní (odstránení hráči + suspendovaní)
                showRemovedSection && React.createElement(
                    ...
                    React.createElement(
                        ...
                        suspendedForBlueCard.map((player, idx) => {
                            const suspensionInfo = suspendedPlayers?.[team.playerDetails.findIndex(p => 
                                p.firstName === player.firstName && 
                                p.lastName === player.lastName && 
                                p.jerseyNumber === player.jerseyNumber
                            )];
                        ...
                        removedPlayers
                            ...
                            .map((player, idx) => {
                                ...
                                if (isMatchScheduled) {
                                    onClickHandler = () => restorePlayerToRoster(player, teamType, teamDetailsForCallbacks);
                        ...
                        removedMenStaff.map((member, idx) => {
                            ...
                            if (isMatchScheduled) {
                                onClickHandler = () => restoreStaffToRoster(member, teamType, teamDetailsForCallbacks, 'men');
                                cursorClass = 'cursor-pointer hover:bg-blue-50';
                            }
                        ...
                        removedWomenStaff.map((member, idx) => {
                            ...
                            if (isMatchScheduled) {
                                onClickHandler = () => restoreStaffToRoster(member, teamType, teamDetailsForCallbacks, 'women');
                                cursorClass = 'cursor-pointer hover:bg-blue-50';
                            }
        ...
        return React.createElement(
            ...
            React.createElement(EditStaffModal, {
                ...
                onRemove: removeStaffFromRoster,
                member: staffToEdit,
                firstName: editStaffFirstName,
                lastName: editStaffLastName,
