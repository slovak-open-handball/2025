...
const TeamMembersList = ({ teamName, categoryName, teamType, timerRef, onMappedNameUpdate, matchId, periodDuration: propPeriodDuration, blueCardSuspensions: propBlueCardSuspensions }) => {
    ...
    // 🔥 OPRAVENÝ VÝPOČET VYLÚČENÝCH ČLENOV - BERIE DO ÚVAHY PERIÓDY
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            members.forEach((member) => {
                ...
                // Zozbierame všetky vylúčenia pre tohto člena
                const exclusions = [];
                ...
                snapshot.forEach((doc) => {
                    ...
                    if (event.team === teamType && 
                        ...
                        // 🔥 DÔLEŽITÉ: Udalosti majú totalTime v SEKUNDÁCH (celkový čas zápasu)
                        // Musíme zistiť, v ktorej perióde k vylúčeniu došlo
                        const exclusionTotalTime = event.totalTime || 0;
                        
                        // 🔥 ZISTÍME PERIÓDU VYLÚČENIA
                        let exclusionPeriod = event.period || 1;
                        // Ak nemáme period v evente, odvodíme ho z totalTime
...
const MatchDetailView = ({ match, teamNames, onBack, hallInfo, categoryDrawColors, groupsData, allMatches, currentMatchIndex, onNavigate, onMatchUpdate }) => {
    ...
    const renderMatchEvents = () => {
        ...
        // 🔥 FUNKCIA: Výpočet skóre v čase udalosti (pre góly) - upravená pre zoznam zoradený od najstaršej
        const getScoreAtEvent = (eventsSortedAsc, targetEvent) => {
            if (targetEvent.eventType !== 'goal') return null;
            
            let homeGoals = 0;
            let awayGoals = 0;
            
            // Prechádzame udalosti od začiatku až po aktuálnu udalosť
            for (let i = 0; i < eventsSortedAsc.length; i++) {
                const event = eventsSortedAsc[i];
                
                // Započítame gól pred aktuálnou udalosťou
        ...
        // 🔥 VYTVORÍME ZORADENÝ ZOZNAM UDALOSTÍ OD NAJSTARŠEJ PO NAJNOVŠIU
        const eventsSortedAsc = [...matchEvents].sort((a, b) => {
            ...
            // Ak je čas rovnaký, použijeme timestamp
                    ...
                    matchEvents.length === 0 ?
                        ...
                        React.createElement(
                            ...
                            React.createElement(
                                'tbody',
                                { className: 'divide-y divide-gray-100' },
                                // 🔥 POUŽIJEME ZORADENÝ ZOZNAM eventsSortedAsc
                                eventsSortedAsc.map((event, idx) => {
                                    const isHomeEvent = event.team === 'home';
                                    const isGoal = event.eventType === 'goal';
                                    // 🔥 POČÍTAME SKÓRE ZO ZORADENÉHO ZOZNAMU
                                    const score = isGoal ? getScoreAtEvent(eventsSortedAsc, event) : null;
