  ...
  const ToggleSwitch = ({ isOn, onToggle, disabled }) => {
    return React.createElement(
      'button',
      {
        ...
        onClick: onToggle,
        disabled: disabled,
        className: `relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${isOn ? 'bg-green-500' : 'bg-gray-300'} ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`,
        style: isOn ? { backgroundColor: '#22c55e' } : {} // priamy štýl ako záloha
      },
