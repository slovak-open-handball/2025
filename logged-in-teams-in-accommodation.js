...
const AddGroupsApp = ({ userProfileData }) => {
    ...
    const categoriesData = categories.map(category => {
        ...
        // Zoskupiť tímy podľa ubytovní
        const teamsByAccommodation = {};
        assignedTeamsInCategory.forEach(team => {
            const accommodationName = team.assignedPlace || 'Nepriradené';
            if (!teamsByAccommodation[accommodationName]) {
                teamsByAccommodation[accommodationName] = [];
            }
            teamsByAccommodation[accommodationName].push(team);
        });
