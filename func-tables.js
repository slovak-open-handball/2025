...
(function() {
    ...
    function calculateHeadToHeadAdvanced(teamA, teamB, groupMatches) {
        let teamAScore = 0;
        let teamBScore = 0;
        let teamAWins = 0;
        let teamBWins = 0;
        ...
        if (!teamAName || !teamBName) {
            log(`⚠️ calculateHeadToHeadAdvanced: chýba názov tímu pre porovnanie`);
            return { teamAScore, teamBScore, teamAWins, teamBWins };
        }
        ...
        if (!foundMatch) {
            ...
            // Vypíšeme debug info o dostupných zápasoch
    ...
    function getGroupType(categoryName, groupName) {
        if (!groupsCache && !groupsCacheLoaded) {
            // Asynchrónne načítanie, ale vrátime false (predvolene základná)
            loadGroupsData().catch(e => error(e));
        ...
        let categoryId = null;
        for (const [catId, catData] of Object.entries(window.categoryIdMap || {})) {
            if (catData === categoryName) {
                categoryId = catId;
                break;
            }
        }
        ...
        if (!categoryId) {
            // Skúsime nájsť priamo v groupsCache
    ...
    // V createGroupTable, po zmapovaní názvov tímov, vytvorte nové pole zápasov s mapovanými názvami
    ...
    function createGroupTable(categoryName, groupName) {
        ...
        // 🔥 NAČÍTANIE TYPU SKUPINY
        const groupType = getGroupTypeSync(categoryName, groupName);
        log(`📌 [${categoryName} - ${groupName}] Typ skupiny: ${groupType}`);
        ...
        const mappedMatchesForComparison = [];
        
        for (const match of allGroupMatches) {
            // Nájdeme mapované názvy tímov
            ...
            // Pridáme do poľa pre porovnanie s mapovanými názvami
            mappedMatchesForComparison.push({
                ...match,
                homeTeamName: homeTeamName,
                awayTeamName: awayTeamName,
                homeScore: homeScore,
    ...
    // ============================================================
    // ÚPLNE OPRAVENÁ FUNKCIA: findMatchBetweenNamedTeams
    // Hľadá zápas priamo v matchesData podľa názvov tímov
    // ============================================================
    function findMatchBetweenNamedTeams(teamAName, teamBName, categoryName, excludeGroupName = null) {
        ...
        for (const match of allMatches) {
            ...
            if (excludeGroupName && match.categoryName === categoryName && match.groupName === excludeGroupName) continue;
            
            // Kontrola kategórie
            if (match.categoryName !== categoryName) continue;
            
            // Získame názvy tímov pre tento zápas
            // Najprv skúsime priamo z matcha
            let homeTeamName = match.homeTeamIdentifier;
            let awayTeamName = match.awayTeamIdentifier;
            
            // Skúsime mapovať, ak je to identifikátor
            ...
            // 🔥 KĽÚČOVÉ: Ak sme v základnej skupine, názvy už môžu byť konečné
            // (napr. "HK Slovan Duslo Šaľa A" namiesto "U12 D 1A")
            ...
            if (hasTeamA && hasTeamB) {
                log(`      ✅ Nájdený zápas v skupine: ${match.groupName} (stav: ${match.status})`);
                log(`         Domáci: ${homeTeamName}`);
                log(`         Hostia: ${awayTeamName}`);
    ...
    function createAdvancedGroupTable(categoryName, groupName, baseGroupName = null) {
        ...
        // 🔥 NOVÉ: Získame všetky nadstavbové skupiny v kategórii a aktuálny index
        const allAdvancedGroups = getAdvancedGroupsInCategory(categoryName);
        const currentIdx = allAdvancedGroups.indexOf(groupName);
        if (currentIdx === -1) {
            log(`⚠️ createAdvancedGroupTable: Skupina ${groupName} nie je v zozname nadstavbových skupín`);
            // Pokračujeme, ale možno to nie je nadstavbová
        }
        ...
        // Získame VŠETKY základné skupiny
        let allBaseGroups = [];
        if (categoryId && groupsCache[categoryId]) {
        ...
        if (allBaseGroupsFullyCompleted.length === 0) {
            log(`❌ NADSTAVBOVÁ SKUPINA ${groupName} NEMÔŽE BYŤ VYHODNOTENÁ - žiadna základná skupina nie je 100% dokončená`);
            return null;
        }
        ...
        // 🔥 VYTVORÍME MAPOVANIE PRE RÝCHLE VYHĽADÁVANIE NÁZVOV
        const teamNameMap = new Map();
        for (const team of teamsInAdvanced) {
            teamNameMap.set(team.originalId, team.name);
            if (team.id !== team.originalId) {
                teamNameMap.set(team.id, team.name);
            }
        }
        
        // Zber prenesených výsledkov - pridané do samostatného poľa, ktoré sa použije aj pre porovnanie
        const transferredMatches = [];
        const processedPairs = new Set();
        const allMatchesForComparison = []; // 🔥 NOVÉ PRÁZDNA POLE
        
        // 🔥 NAJPRV PRIDÁME VŠETKY ZÁPASY Z NADSTAVBOVEJ SKUPINY SO SPRÁVNYMI NÁZVA MI
        for (const match of advancedMatches) {
            // Získame mapované názvy tímov
            let homeFinalName = teamNameMap.get(match.homeTeamIdentifier) || match.homeTeamIdentifier;
            let awayFinalName = teamNameMap.get(match.awayTeamIdentifier) || match.awayTeamIdentifier;
            
            // Ak je to identifikátor, skúsime mapovať
            ...
            allMatchesForComparison.push({
                ...
                forfeitResult: match.forfeitResult,
                isTransferred: false
            });
        }
        
        // ============================================================
        // 🔥 PRENESENIE ZO ZÁKLADNÝCH SKUPÍN (pôvodná logika)
        // ============================================================
        if (carryOverEnabled) {
            for (const baseGroup of allBaseGroupsFullyCompleted) {
                const baseTable = createGroupTable(categoryName, baseGroup);
                ...
                for (const match of completedBaseMatches) {
                    ...
                    // Nájdenie finálnych názvov tímov z baseTable (kde už sú mapované)
        ...
        // ============================================================
        // 🔥 NOVÉ: PRENESENIE Z PREDCHÁDZAJÚCICH NADSTAVBOVÝCH SKUPÍN (ABECEDNE)
        // ============================================================
        if (carryOverEnabled && currentIdx > 0) {
            // Prejdeme všetky nadstavbové skupiny, ktoré sú abecedne pred aktuálnou
            for (let i = 0; i < currentIdx; i++) {
                const prevGroupName = allAdvancedGroups[i];
                // Získame tabuľku predchádzajúcej skupiny (rekurzívne volanie)
                const prevTable = createAdvancedGroupTable(categoryName, prevGroupName);
                if (!prevTable || prevTable.completionPercentage !== 100) {
                    log(`⏳ Predchádzajúca nadstavbová skupina ${prevGroupName} nie je dokončená, preskakujem prenos do ${groupName}`);
                    continue;
                }
                
                // Pre každý zápas v predchádzajúcej skupine
                for (const match of prevTable.matches) {
                    // Ak sú oba tímy v aktuálnej skupine
                    const homeInCurrent = teamsInAdvanced.some(t => t.name === match.homeTeamName);
                    const awayInCurrent = teamsInAdvanced.some(t => t.name === match.awayTeamName);
                    if (homeInCurrent && awayInCurrent) {
                        const pairKey = match.homeTeamName < match.awayTeamName ?
                            `${match.homeTeamName}|${match.awayTeamName}` :
                            `${match.awayTeamName}|${match.homeTeamName}`;
                        if (!processedPairs.has(pairKey)) {
                            // Pridáme prenesený zápas z predchádzajúcej nadstavbovej skupiny
                            const transferredMatch = {
                                matchId: match.id || `transferred_adv_${Date.now()}_${Math.random()}`,
                                fromGroup: prevGroupName,
                                homeScore: match.homeScore || 0,
                                awayScore: match.awayScore || 0,
                                homeTeam: match.homeTeamName,
                                awayTeam: match.awayTeamName,
                                isTransferred: true,
                                fromAdvancedGroup: true,
                                homeTeamIdentifier: match.homeTeamName,
                                awayTeamIdentifier: match.awayTeamName,
                            ...
                            allMatchesForComparison.push(transferredMatch);
                            processedPairs.add(pairKey);
                            log(`🔄 Prenesený zápas z nadstavbovej skupiny ${prevGroupName}: ${match.homeTeamName} ${match.homeScore}:${match.awayScore} ${match.awayTeamName}`);
                        }
                    }
                }
            }
        ...
        // ============================================================
        // APLIKÁCIA PRENESENÝCH ZÁPASOV NA ŠTATISTIKY
        // ============================================================
        // Aplikujeme prenesené zápasy (zo základných aj z nadstavbových) na štatistiky
        for (const transferred of transferredMatches) {
            const homeTeamStats = teamsInAdvanced.find(t => t.name === transferred.homeTeam);
            const awayTeamStats = teamsInAdvanced.find(t => t.name === transferred.awayTeam);
            
            if (homeTeamStats && awayTeamStats) {
                homeTeamStats.played++;
                awayTeamStats.played++;
                homeTeamStats.goalsFor += transferred.homeScore;
                homeTeamStats.goalsAgainst += transferred.awayScore;
                awayTeamStats.goalsFor += transferred.awayScore;
                awayTeamStats.goalsAgainst += transferred.homeScore;
                
                if (transferred.homeScore > transferred.awayScore) {
                    homeTeamStats.wins++;
                    awayTeamStats.losses++;
                    homeTeamStats.points += pointsForWin;
                } else if (transferred.awayScore > transferred.homeScore) {
                    awayTeamStats.wins++;
                    homeTeamStats.losses++;
                    awayTeamStats.points += pointsForWin;
                } else {
        ...
        // 🔥 POUŽIJEME allMatchesForComparison (ktoré obsahuje aj prenesené zápasy) pre správne porovnanie vzájomných zápasov
        const sortedTeams = [...teamsInAdvanced].sort((a, b) => {
            return compareTeams(a, b, allMatchesForComparison, tableSettings.sortingConditions);
        });
        
        // 🔥 FINÁLNE: Vytvoríme zoznam zápasov na zobrazenie
        const allMatchesForDisplay = [];
        
        // Normálne zápasy
        for (const match of allMatchesForComparison) {
            if (match.isTransferred) continue;
        ...
        // Prenesené zápasy (zo základných aj z nadstavbových)
        for (const transferred of transferredMatches) {
            allMatchesForDisplay.unshift({
                id: transferred.matchId || `transferred_${Date.now()}_${Math.random()}`,
                ...
                fromAdvancedGroup: transferred.fromAdvancedGroup || false
            });
        }
        ...
        return {
            category: categoryName,
            group: groupName,
            groupType: groupTypeFromDB,
            baseGroup: allBaseGroupsFullyCompleted.join(', '),
            carryOverPoints: carryOverEnabled,
            teams: sortedTeams,
            matches: allMatchesForDisplay,
            completedMatches: allMatchesForComparison.filter(m => !m.isTransferred && m.status === 'completed'),
    ...
    // Získa zoznam všetkých nadstavbových skupín v danej kategórii (zoradené abecedne)
    function getAdvancedGroupsInCategory(categoryName) {
        if (!groupsCache) return [];
        let categoryId = window.categoryIdMap?.[categoryName];
        if (!categoryId) {
            for (const [catId, groups] of Object.entries(groupsCache)) {
                if (catId === categoryName || window.categoryIdMap?.[catId] === categoryName) {
                    categoryId = catId;
                    break;
                }
            }
        ...
        const groups = groupsCache[categoryId];
        const advanced = groups.filter(g => g.type === 'nadstavbová skupina').map(g => g.name);
        advanced.sort((a, b) => a.localeCompare(b));
        return advanced;
    }
    
    // Zistí, či je možné vyhodnotiť danú nadstavbovú skupinu (všetky závislosti sú splnené)
    function isAdvancedGroupEvaluable(categoryName, groupName) {
        const advancedGroups = getAdvancedGroupsInCategory(categoryName);
        const idx = advancedGroups.indexOf(groupName);
        if (idx === -1) return false;
    
        const categorySetting = categorySettingsCache[categoryName];
        const carryOverEnabled = categorySetting?.carryOverPoints ?? false;
    
        // Ak nie je zapnuté prenášanie, považujeme skupinu za vyhodnotiteľnú vždy (alebo podľa pôvodnej logiky)
        if (!carryOverEnabled) {
            // Tu môžete pridať ďalšie podmienky, ak treba
            return true;
        }
    
        // 1. Skontrolujeme, či sú všetky základné skupiny dokončené (ak carryOver vyžaduje)
        const baseGroups = getAllBaseGroupsInCategory(categoryName);
        for (const baseGroup of baseGroups) {
            if (!isGroupFullyCompleted(categoryName, baseGroup)) return false;
        }
    
        // 2. Skontrolujeme predchádzajúce nadstavbové skupiny (abecedne pred aktuálnou)
        for (let i = 0; i < idx; i++) {
            const prevGroup = advancedGroups[i];
            const prevTable = createAdvancedGroupTable(categoryName, prevGroup);
            if (!prevTable || prevTable.completionPercentage !== 100) {
                return false;
            }
        }
    ...
    // Získa názvy všetkých základných skupín v kategórii
    function getAllBaseGroupsInCategory(categoryName) {
        if (!groupsCache) return [];
        let categoryId = window.categoryIdMap?.[categoryName];
        if (!categoryId) {
            for (const [catId, groups] of Object.entries(groupsCache)) {
        ...
        if (!categoryId || !groupsCache[categoryId]) return [];
        return groupsCache[categoryId].filter(g => g.type === 'základná skupina').map(g => g.name);
    }
    ...
    function printGroupTable(categoryName, groupName, baseGroupName = null) {
        const getGroupTypeAndPrint = async () => {
            ...
            // 🔥 VÝPIS TABUĽKY - UŽ POUŽÍVAME NÁZVY Z TABLE, KTORÉ SÚ SPRÁVNE ZMAPOVANÉ
            log('\n' + '='.repeat(120));
            ...
            // 🔥 table.teams už majú správne názvy (žiadne ďalšie mapovanie nepotrebujeme)
            ...
            // Výpis zápasov - tiež už majú správne názvy
                ...
                if (normal.length > 0) {
                    ...
                    normal.forEach((match, idx) => {
                        ...
                        if (match.status === 'completed') {
                            ...
                            log(`   ${idx+1}. ⏳ ${match.homeTeamName} vs ${match.awayTeamName} (${dateStr}) - ${getStatusText(match.status)}`);
                        }
                    });
                }
            ...
            if (!isInitialDataLoaded && loadedMatchesCount === totalMatchesCount) {
                ...
                for (const groupKey of advancedGroups) {
                    ...
                    if (baseGroupName) {
                        ...
                        if (isAdvancedGroupEvaluable(category, group)) {
                            log(`   ✅ ${category} - ${group} je vyhodnotiteľná - vyhodnocujem...`);
                            createAdvancedGroupTable(category, group);
                        } else {
                            log(`   ⏳ ${category} - ${group} čaká na dokončenie predchádzajúcich skupín`);
                        }
                    }
                }
            }
