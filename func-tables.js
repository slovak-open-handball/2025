...
//        console.log(`✅ [${cleanCategory} - ${fullGroupName}] Skupina JE PRIpravenÁ OKAMŽITE!`);
...
//            console.log(`🎉 Spúšťam OKAMŽITÉ nahradenie pre skupinu ${fullGroupName}...`);
...
// ** NOVÉ: Sledovanie stavu skupín pre automatické vrátenie **
let groupStatusCache = new Map(); // Ukladá stav pripravenosti pre každú skupinu
let groupContentCache = new Map(); // Ukladá pôvodný obsah pre každú skupinu

// Funkcia na uloženie pôvodného obsahu elementov pre danú skupinu
function saveOriginalContentForGroup(category, groupLetter, element, originalText) {
    const groupKey = `${category}|${groupLetter.toUpperCase()}`;
    
    if (!groupContentCache.has(groupKey)) {
        groupContentCache.set(groupKey, new Map());
    }
    
    const elementCache = groupContentCache.get(groupKey);
    const elementId = element.id || `elem_${Date.now()}_${Math.random()}`;
...
// Funkcia na obnovenie pôvodného obsahu pre skupinu, ktorá už nie je pripravená
function revertGroupToOriginal(category, groupLetter) {
    const groupKey = `${category}|${groupLetter.toUpperCase()}`;
    
    if (!groupContentCache.has(groupKey)) {
//        console.log(`ℹ️ Žiadny uložený obsah pre skupinu ${groupKey}`);
        return false;
    }
    ...
    for (const [elementId, data] of elementCache.entries()) {
        try {
            const element = data.element;
            if (element && element.parentNode && element.textContent !== data.originalText) {
                element.textContent = data.originalText;
                revertedCount++;
                
                // Odstránime atribúty súvisiace s nahradením
                element.removeAttribute('data-original-identifier');
                element.removeAttribute('data-team-category');
                element.removeAttribute('data-team-position');
                element.removeAttribute('data-team-group');
                element.removeAttribute('data-team-name');
                element.removeAttribute('data-replaced-100-percent');
                element.removeAttribute('data-replaced-at');
            }
        } catch (error) {
            console.error(`❌ Chyba pri obnove elementu ${elementId}:`, error);
        }
    }
    
    if (revertedCount > 0) {
//        console.log(`🔄 Obnovených ${revertedCount} elementov pre skupinu ${groupKey} (už nemá 100% odohraných zápasov)`);
    }
    
    // Vymažeme cache pre túto skupinu
    groupContentCache.delete(groupKey);
    
    return revertedCount > 0;
}

// ** NOVÁ FUNKCIA: Kontrola všetkých skupín a vrátenie tých, ktoré už nie sú pripravené **
function checkAndRevertUnreadyGroups() {
    const allGroupKeys = new Set();
    
    // Zozbierame všetky skupiny z cache
    for (const key of groupContentCache.keys()) {
        allGroupKeys.add(key);
    }
    
    // Zozbierame všetky skupiny z processedGroups
    for (const key of processedGroups.keys()) {
        allGroupKeys.add(key);
    }
    ...
    for (const groupKey of allGroupKeys) {
        const [category, groupLetter] = groupKey.split('|');
        
        // Skontrolujeme aktuálnu pripravenosť skupiny
        const isReady = isGroupReadyForReplacement(category, groupLetter);
        const wasReady = processedGroups.get(groupKey)?.isReady || false;
        
        // Ak bola pripravená a už nie je → vrátime pôvodný obsah
        if (wasReady && !isReady) {
//            console.log(`⚠️ Skupina ${groupKey} stratila 100% status! Vraciam pôvodné identifikátory...`);
            const reverted = revertGroupToOriginal(category, groupLetter);
            if (reverted) {
                anyReverted = true;
                
                // Aktualizujeme stav v processedGroups
                processedGroups.set(groupKey, {
                    isReady: false,
                    percentage: 0,
                    lastCheck: Date.now(),
...
// ** UPRAVENÁ FUNKCIA: isGroupReadyForReplacement - s ukladaním pôvodného obsahu **
...
window.isGroupReadyForReplacement = function(category, groupLetter) {
    ...
    if (!groupTable) {
        // Aktualizujeme stav na false
        if (processedGroups.get(groupKey)?.isReady !== false) {
            processedGroups.set(groupKey, {
                isReady: false,
                percentage: 0,
                lastCheck: Date.now(),
            ...
            // Skúsime vrátiť pôvodný obsah
            revertGroupToOriginal(cleanCategory, groupLetter);
        }
        return false;
    }
    ...
    // 2. Podmienka 1: Všetky zápasy musia byť odohrané (100%)
    if (completionPercentage < 100) {
        // Ak bola predtým pripravená a už nie je → vrátime obsah
        if (processedGroups.get(groupKey)?.isReady === true) {
//            console.log(`⚠️ Skupina ${groupKey} už nemá 100% (${completionPercentage}%)! Vraciam pôvodné identifikátory...`);
            revertGroupToOriginal(cleanCategory, groupLetter);
        }
    ...
    if (!allEventsLoaded) {
        if (processedGroups.get(groupKey)?.isReady === true) {
//            console.log(`⚠️ Skupina ${groupKey} už nemá načítané všetky udalosti! Vraciam pôvodné identifikátory...`);
            revertGroupToOriginal(cleanCategory, groupLetter);
        }
    ...
    if (hasInProgressMatches) {
        if (processedGroups.get(groupKey)?.isReady === true) {
//            console.log(`⚠️ Skupina ${groupKey} má prebiehajúce zápasy! Vraciam pôvodné identifikátory...`);
            revertGroupToOriginal(cleanCategory, groupLetter);
        }
    ...
    // Ak bola predtým nepripravená a teraz je pripravená, spustíme nahradenie
    if (!wasReady) {
//        console.log(`✅ [${cleanCategory} - ${fullGroupName}] Skupina JE PRIPRAVENÁ!`);
...
// ** UPRAVENÁ FUNKCIA: performPartialReplacement - s ukladaním pôvodného obsahu **
const originalPerformPartialReplacement = window.performPartialReplacement || performPartialReplacement;
    ...
    for (const idInfo of identifiersToReplace) {
        ...
        if (teamName && teamName !== idInfo.identifier && teamName !== idInfo.originalIdentifier) {
            ...
            for (const textNode of textNodes) {
                ...
                const newText = originalText.replace(regex, teamName);
                if (newText !== originalText) {
                    // 🔴 ULOŽÍME PÔVODNÝ OBSAH pre prípadné vrátenie
                    const parentElement = textNode.parentElement;
                    if (parentElement) {
                        saveOriginalContentForGroup(
                            idInfo.category, 
                            idInfo.groupLetter, 
                            parentElement, 
                            originalText
            ...
            const inputs = document.querySelectorAll('input, textarea, [contenteditable="true"]');
            for (const input of inputs) {
                let originalValue = input.value || input.textContent;
                if (originalValue && originalValue.includes(idInfo.originalIdentifier)) {
                    const newValue = originalValue.replace(regex, teamName);
...
// ** NOVÁ FUNKCIA: Pravidelná kontrola všetkých skupín (každých 5 sekúnd) **
let groupStatusCheckInterval = null;

function startGroupStatusMonitoring(intervalSeconds = 5) {
    if (groupStatusCheckInterval) {
        clearInterval(groupStatusCheckInterval);
    }
    
    groupStatusCheckInterval = setInterval(() => {
        // Skontrolujeme všetky skupiny, ktoré máme v cache
        const allGroupKeys = new Set();
        ...
        for (const groupKey of allGroupKeys) {
            ...
            // Znovu skontrolujeme pripravenosť skupiny
            const isReady = window.isGroupReadyForReplacement(category, groupLetter);
            const wasReady = processedGroups.get(groupKey)?.isReady || false;
            
            // Ak sa zmenil stav, zaznamenáme
            if (isReady !== wasReady) {
                anyChange = true;
            }
        }
        
        // Ak nastala zmena, môžeme logovať
        if (anyChange) {
//            console.log('🔄 Zmena v stave skupín bola zaznamenaná');
        }
    }, intervalSeconds * 1000);
    
//    console.log(`👁️ Spustené monitorovanie skupín (kontrola každých ${intervalSeconds} sekúnd)`);
}

function stopGroupStatusMonitoring() {
    if (groupStatusCheckInterval) {
        clearInterval(groupStatusCheckInterval);
        groupStatusCheckInterval = null;
//        console.log('⏹️ Monitorovanie skupín zastavené');
    }
}

// ** PRIDANIE NOVÝCH FUNKCIÍ DO teamNameReplacer **
if (window.teamNameReplacer) {
    window.teamNameReplacer.checkAndRevertUnreadyGroups = checkAndRevertUnreadyGroups;
    window.teamNameReplacer.revertGroupToOriginal = revertGroupToOriginal;
    window.teamNameReplacer.startGroupStatusMonitoring = startGroupStatusMonitoring;
    window.teamNameReplacer.stopGroupStatusMonitoring = stopGroupStatusMonitoring;
    window.teamNameReplacer.getGroupStatusCache = () => {
        const result = {};
        for (const [key, value] of processedGroups.entries()) {
            result[key] = value;
        }
    ...
    window.teamNameReplacer.getGroupContentCache = () => {
        const result = {};
        for (const [key, elements] of groupContentCache.entries()) {
            result[key] = elements.size;
        }
        return result;
    };
}

// Automatické spustenie monitorovania
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        startGroupStatusMonitoring(5);
    });
} else {
    startGroupStatusMonitoring(5);
}
...
// ============================================================
// FUNKCIA NA MANUÁLNE VYNÚTENIE KONTROLY
// ============================================================
function forceCheckAllGroups() {
//    console.log('🔍 Vynucujem kontrolu všetkých skupín...');
    
    // Znovu skontrolujeme všetky skupiny v processedGroups
    for (const [groupKey, oldStatus] of processedGroups.entries()) {
        const [category, groupLetter] = groupKey.split('|');
        const isReady = window.isGroupReadyForReplacement(category, groupLetter);
        
        if (oldStatus.isReady && !isReady) {
//            console.log(`⚠️ Skupina ${groupKey} už nie je pripravená!`);
            revertGroupToOriginal(category, groupLetter);
        }
    }
    
    // Tiež skontrolujeme všetky skupiny v cache
    for (const groupKey of groupContentCache.keys()) {
        const [category, groupLetter] = groupKey.split('|');
        const isReady = window.isGroupReadyForReplacement(category, groupLetter);
...
// Pridáme do window
window.forceCheckAllGroups = forceCheckAllGroups;

console.log('✅ Sledovanie skupín aktívne - ak skupina stratí 100% status, obsah sa automaticky vráti do pôvodného stavu');
