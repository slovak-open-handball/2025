...
(function() {
    ...
    function findMatchBetweenTeamsInOtherGroup(teamAName, teamBName, currentCategory, currentGroupName, excludeGroupName) {
        ...
        for (const match of allMatches) {
            ...
            if (hasTeamA && hasTeamB) {
                ...
                if (match.status === 'completed') {
                    ...
                    console.log(`      ✅ POUŽITÝ výsledok: ${teamAScore}:${teamBScore}`);
        ...
        console.log(`      ❌ Žiadny dokončený zápas medzi "${teamAName}" a "${teamBName}" nebol nájdený.`);
        return null;
    }
    ...
    function createGroupTable(categoryName, groupName) {
        ...
        if (carryOverMatches) { 
            if (!processedCarryOverGroups.has(groupCarryKey)) {
                ...
                if (missingPairs.length > 0) {
                    for (const pair of missingPairs) {
                        ...
                        if (transferredMatch && transferredMatch.isTransferred) {
                            console.log(`   ✅ Prenesený výsledok: ${teamA.name} ${transferredMatch.homeScore}:${transferredMatch.awayScore} ${teamB.name}`);
                ...
                console.log(`✅ Skupina ${categoryName} - ${groupName} označená ako spracovaná`);
            } else {
                console.log(`ℹ️ Skupina ${categoryName} - ${groupName} už bola spracovaná, preskakujem prenášanie výsledkov`);
            }
    ...
    function getTeamNameForAdvancedGroup(teamNameFromGroup, category, groupLetter, position) {
        ...
        if (!looksLikeIdentifier) {
            ...
            console.log(`   ℹ️ "${teamNameFromGroup}" nie je identifikátor (nemá formát A2 alebo 2A), používam pôvodný názov`);
            return teamNameFromGroup;
        }
        
        console.log(`   📛 Spracovávam identifikátor: "${teamNameFromGroup}"`);
        console.log(`   📍 Volám window.matchTracker.getTeamNameByDisplayId("${teamNameFromGroup}")...`);
        
        const mappedName = window.matchTracker.getTeamNameByDisplayId(teamNameFromGroup);
        
        if (mappedName && mappedName !== teamNameFromGroup) {
            console.log(`   ✅ Mapovanie tímu: "${teamNameFromGroup}" → "${mappedName}"`);
            return mappedName;
        }
        
        // Ak funkcia vrátila null alebo rovnaký názov, použijeme pôvodný
        if (!mappedName) {
            console.log(`   ⚠️ getTeamNameByDisplayId vrátila null pre "${teamNameFromGroup}", používam pôvodný názov`);
        } else if (mappedName === teamNameFromGroup) {
            console.log(`   ℹ️ Názov zostáva rovnaký: "${teamNameFromGroup}"`);
        }
        return teamNameFromGroup;
    }
    ...
    function createAdvancedGroupTable(categoryName, groupName, baseGroupName) {
        ...
        // PRE FINÁLOVÚ SKUPINU (F) - hľadáme všetky základné skupiny
        const isFinalGroup = groupName.toLowerCase().includes('skupina f') || groupName.toLowerCase() === 'skupina f';
        
        let allBaseGroupTables = [];
        let allBaseTeams = [];
        
        if (isFinalGroup) {
            // Zoznam základných skupín A-E
            const baseGroupLetters = ['A', 'B', 'C', 'D', 'E'];
            
            console.log(`🎯 Nadstavbová skupina ${groupName} je FINÁLOVÁ - budem hľadať výsledky zo všetkých základných skupín`);
            
            for (const letter of baseGroupLetters) {
                const baseGroupNameTry = `skupina ${letter}`;
                const baseGroupTable = createGroupTable(categoryName, baseGroupNameTry);
                if (baseGroupTable && baseGroupTable.completionPercentage === 100) {
                    ...
                    console.log(`   ✅ Základná skupina ${baseGroupNameTry} je 100% dokončená, načítaných ${baseGroupTable.teams.length} tímov`);
                } else if (baseGroupTable) {
                    console.log(`   ⏳ Základná skupina ${baseGroupNameTry} má len ${baseGroupTable.completionPercentage}% dokončených`);
                } else {
                    console.log(`   ⚠️ Základná skupina ${baseGroupNameTry} neexistuje`);
                }
            }
            
            if (allBaseGroupTables.length === 0) {
                console.log(`❌ Žiadna základná skupina nie je 100% dokončená, nemôžem vyhodnotiť finálovú skupinu`);
                return null;
            }
        } else {
            ...
            allBaseGroupTables = [baseGroupTable];
            allBaseTeams = [...baseGroupTable.teams];
        }
        
        console.log(`✅ Základné skupiny sú 100% dokončené, vyhodnocujem nadstavbovú skupinu ${groupName}...`);
        
        // Zvyšok funkcie zostáva podobný...
        const categorySetting = categorySettingsCache[categoryName];
        ...
        if (advancedMatches.length === 0) {
            console.log(`❌ Žiadne zápasy pre nadstavbovú skupinu ${groupName}`);
            return null;
        }
        
        let teamsInAdvanced = getTeamsInGroupFromAllMatches(advancedMatches);
        
        // Mapovanie tímov pre finálovú skupinu - použijeme všetky základné skupiny
        if (isFinalGroup) {
            teamsInAdvanced = mapAdvancedGroupTeamsFinal(teamsInAdvanced, categoryName, allBaseGroupTables);
        } else {
            teamsInAdvanced = mapAdvancedGroupTeams(teamsInAdvanced, categoryName, allBaseGroupTables[0], baseGroupName);
        }
        
        // Mapovanie bodov zo základných skupín
        const basePointsMap = new Map();
        if (carryOverPoints) {
            for (const baseGroupTable of allBaseGroupTables) {
                for (const baseTeam of baseGroupTable.teams) {
                    basePointsMap.set(baseTeam.name, (basePointsMap.get(baseTeam.name) || 0) + baseTeam.points);
                }
            }
        }
        
        // Získame všetky ODOHRANÉ zápasy zo všetkých základných skupín
        const allBaseMatches = [];
        for (const baseGroupTable of allBaseGroupTables) {
            const baseMatches = getGroupMatches(categoryName, baseGroupTable.group);
            allBaseMatches.push(...baseMatches.filter(m => m.status === 'completed'));
        }
        
        // Vytvoríme mapu ID tímov v nadstavbovej skupine na ich pôvodné ID v základných skupinách
        const advancedTeamToBaseInfo = new Map(); // { advancedTeamId: { baseTeamId, baseGroupName, teamName } }
        
        for (const advancedTeam of teamsInAdvanced) {
            // Skúsime nájsť tím v základných skupinách podľa názvu
            let foundBaseTeam = null;
            let foundBaseGroupName = null;
            
            for (const baseGroupTable of allBaseGroupTables) {
                const baseTeam = baseGroupTable.teams.find(t => t.name === advancedTeam.name);
                if (baseTeam) {
                    foundBaseTeam = baseTeam;
                    foundBaseGroupName = baseGroupTable.group;
                    break;
                }
            }
            
            if (foundBaseTeam) {
                advancedTeamToBaseInfo.set(advancedTeam.id, {
                    baseTeamId: foundBaseTeam.id,
                    baseGroupName: foundBaseGroupName,
                    teamName: foundBaseTeam.name
                });
            } else {
                console.log(`⚠️ Tím "${advancedTeam.name}" nebol nájdený v žiadnej základnej skupine`);
            }
        }
        
        // Spracujeme výsledky zo základných skupín
        const transferredMatches = [];
        const processedPairs = new Set(); // Aby sme nepridali rovnaký zápas dvakrát
        ...
        if (carryOverMatches) {
            for (let i = 0; i < teamsInAdvanced.length; i++) {
                for (let j = i + 1; j < teamsInAdvanced.length; j++) {
                    ...
                    const baseInfoA = advancedTeamToBaseInfo.get(teamA.id);
                    const baseInfoB = advancedTeamToBaseInfo.get(teamB.id);
                    
                    if (!baseInfoA || !baseInfoB) continue;
                    
                    // Hľadáme zápas medzi týmito dvoma tímami v základných skupinách
                    let baseMatch = null;
                    
                    // Najprv skúsime nájsť zápas v rovnakej základnej skupine (ak hrali spolu)
                    if (baseInfoA.baseGroupName === baseInfoB.baseGroupName) {
                        baseMatch = allBaseMatches.find(m => 
                            (m.homeTeamIdentifier === baseInfoA.baseTeamId && m.awayTeamIdentifier === baseInfoB.baseTeamId) ||
                            (m.homeTeamIdentifier === baseInfoB.baseTeamId && m.awayTeamIdentifier === baseInfoA.baseTeamId)
                        );
                    }
                    
                    // Ak nenašli v rovnakej skupine, skúsime nájsť v akejkoľvek skupine (cez getTeamName)
                    if (!baseMatch) {
                        baseMatch = findMatchBetweenTeamsInOtherGroup(
                            teamA.name, teamB.name, categoryName, groupName, groupName
                        );
                    }
                    
                    if (baseMatch && baseMatch.isTransferred) {
                        console.log(`   ✅ Prenesený výsledok (${baseInfoA.baseGroupName}): ${teamA.name} ${baseMatch.homeScore}:${baseMatch.awayScore} ${teamB.name}`);
                        
                        teamA.played++;
                        teamB.played++;
                        teamA.goalsFor += baseMatch.homeScore;
                        teamA.goalsAgainst += baseMatch.awayScore;
                        teamB.goalsFor += baseMatch.awayScore;
                        teamB.goalsAgainst += baseMatch.homeScore;
                        
                        if (baseMatch.homeScore > baseMatch.awayScore) {
                            teamA.wins++;
                            teamB.losses++;
                            if (!carryOverPoints) teamA.points += 2;
                        } else if (baseMatch.awayScore > baseMatch.homeScore) {
                            teamB.wins++;
                            teamA.losses++;
                        ...
                        transferredMatches.push({
                            ...
                            fromGroup: baseMatch.fromGroup || baseInfoA.baseGroupName
                        });
        ...
        // Pridanie bodov zo základných skupín
        if (carryOverPoints) {
        ...
        return {
            category: categoryName,
            group: groupName,
            baseGroup: isFinalGroup ? 'skupiny A-E' : baseGroupName,
    ...
    function mapAdvancedGroupTeamsFinal(teamsInAdvanced, categoryName, allBaseGroupTables) {
        const cleanCategory = cleanCategoryName(categoryName);
        
        console.log(`\n📋 MAPOVANIE TÍMOV PRE FINÁLOVÚ SKUPINU: ${categoryName}`);
        ...
        for (const team of teamsInAdvanced) {
            ...
            if (foundTeam) {
                ...
                if (mappedName && mappedName !== team.name && mappedName !== displayId) {
                    ...
                    console.log(`   ⚠️ getTeamNameByDisplayId vrátila null pre "${displayId}", používam pôvodný názov: "${team.name}"`);
                } else if (mappedName === team.name) {
                    console.log(`   ℹ️ Názov zostáva rovnaký: "${team.name}"`);
                }
            } else {
                console.log(`   ⚠️ Tím "${team.name}" nebol nájdený v žiadnej základnej skupine`);
            }
        }
        ...
        console.log('\n' + '='.repeat(80));
        console.log('✅ MAPOVANIE TÍMOV DOKONČENÉ\n');
    ...
    async function loadCategorySettings() {
        ...
        try {
            ...
            if (categoriesSnap.exists()) {
                ...
                console.log('📋 Načítané nastavenia kategórií (carryOverPoints):', categorySettingsCache);
            }
        } catch (error) {
    ...
    function printGroupTable(categoryName, groupName, baseGroupName = null) {
        ...
        for (const team of table.teams) {
            ...
            if (looksLikeIdentifier(team.name)) {
                ...
                if (mappedName && mappedName !== team.name) {
                    console.log(`   🔄 Mapovanie pre výpis: "${team.name}" → "${mappedName}"`);
                    team.name = mappedName;
                } else if (!mappedName) {
                    console.log(`   ⚠️ Žiadne mapovanie pre: "${team.name}" (funkcia vrátila null)`);
                }
            } else {
                console.log(`   ℹ️ "${team.name}" nie je identifikátor, preskakujem mapovanie`);
            }
        }
        // ============================================================
        ...
        if (table.baseGroup) {
            console.log(`   📌 Základná skupina: ${table.baseGroup}`);
            console.log(`   📌 Prenášanie bodov: ${table.carryOverPoints ? 'ZAPNUTÉ ✅' : 'VYPNUTÉ ❌'}`);
        }
        ...
        if (upcomingMatches.length > 0) {
            ...
            upcomingMatches.forEach((match, idx) => {
                ...
                console.log(`   ${idx+1}. ${mappedHome} vs ${mappedAway} (${dateStr}) - ${getStatusText(match.status)}`);
            });
        }
        if (table.transferredMatches && table.transferredMatches.length > 0) {
            ...
            table.transferredMatches.forEach((match, idx) => {
                console.log(`   ${idx+1}. ${match.homeTeam} ${match.homeScore}:${match.awayScore} ${match.awayTeam} (prenesené)`);
            });
        }
    ...
    function mapAdvancedGroupTeams(teamsInAdvanced, categoryName, baseGroupTable, baseGroupName) {
        ...
        for (const team of teamsInAdvanced) {
            ...
            console.log(`\n🔄 Spracovávam tím: "${team.name}"`);
            console.log(`   📍 Pozícia v základnej skupine ${baseGroupName}: ${position}. miesto`);
            console.log(`   📍 Volám window.matchTracker.getTeamNameByDisplayId("${displayId}")...`);
            ...
            if (mappedName && mappedName !== team.name && mappedName !== displayId) {
                ...
                console.log(`   ⚠️ Žiadna zmena pre: "${team.name}" (funkcia vrátila: ${mappedName})`);
            }
        }
    ...
    function mapTeamForAdvancedGroup(categoryName, baseGroupName, teamId, position) {
        ...
        console.log(`🔄 Mapujem tím na pozícii ${position} v skupine ${baseGroupName}: ${displayId}`);
        
        const mappedName = window.matchTracker.getTeamNameByDisplayId(displayId);
        
        if (mappedName && mappedName !== displayId) {
            console.log(`   ✅ Výsledok mapovania: "${displayId}" → "${mappedName}"`);
            return mappedName;
        }
        
        console.log(`   ⚠️ Mapovanie zlyhalo, používam pôvodný identifikátor: ${teamId}`);
        return teamId;
    }
    ...
    async function initializeMatchTracker() {
        ...
        console.log('🗑️ Cache prenášania výsledkov bola resetovaná');
        ...
        console.log(`📊 Celkový počet zápasov v databáze: ${totalMatchesCount}`);
        ...
        unsubscribeMatches = onSnapshot(matchesRef, (snapshot) => {
            ...
            snapshot.docChanges().forEach(change => {
                ...
                if (change.type === 'added') {
                    ...
                    loadedMatchesCount++;
                    console.log(`📊 Priebeh načítavania: ${loadedMatchesCount}/${totalMatchesCount} zápasov`);
            ...
            if (!isInitialDataLoaded && loadedMatchesCount === totalMatchesCount) {
                ...
                console.log('🎉 VŠETKY ZÁPASY BOLI NAČÍTANÉ! Spúšťam prvotné vyhodnotenie...');
                console.log('📋 Najprv spracujem ZÁKLADNÉ SKUPINY...');
                ...
                console.log(`📊 Nájdených ${baseGroups.size} základných skupín`);
                ...
                for (const groupKey of baseGroups) {
                    ...
                    if (groupTable) {
                        ...
                        console.log(`   📊 ${category} - ${group}: ${completionPercentage}% dokončených`);
                        if (completionPercentage == 100) {
                            console.log(`   ✅ ${category} - ${group} je KOMPLETNÁ (100%)`);
                            processedGroupsInitial.add(groupKey);
                        }
                    }
                }
                
                console.log('📋 Teraz spracujem NADSTAVBOVÉ SKUPINY...');
                ...
                console.log(`📊 Nájdených ${advancedGroups.size} nadstavbových skupín`);
                ...
                for (const groupKey of advancedGroups) {
                    ...
                    if (baseGroupName) {
                        ...
                        if (processedGroupsInitial.has(baseGroupKey)) {
                            console.log(`   ✅ ${category} - ${group} (základná ${baseGroupName} je hotová) - vyhodnocujem...`);
                            createAdvancedGroupTable(category, group, baseGroupName);
                        } else {
                            console.log(`   ⏳ ${category} - ${group} čaká na dokončenie základnej skupiny ${baseGroupName}`);
                        }
                    }
                }
            }
            ...
            if (isInitialDataLoaded && completedMatchChanged && changedMatches.length > 0) {
                console.log(`🏁 Spúšťam prepočet tabuliek (${changedMatches.length} zápasov bolo dohraných)...`);
                ...
                console.log(`📋 Ovplyvnené skupiny: ${Array.from(affectedGroups).join(', ')}`);
                ...
                for (const groupKey of affectedGroups) {
                    ...
                    if (!isAdvancedGroup) {
                        console.log(`   🔄 Prepočítavam základnú skupinu: ${category} - ${group}`);
                        const groupTable = createGroupTable(category, group);
                        
                        if (groupTable && groupTable.completionPercentage == 100) {
                            console.log(`   ✅ ${category} - ${group} je teraz KOMPLETNÁ (100%)`);
                            processedGroupsInitial.add(groupKey);
                            
                            const advancedDependentGroups = findAdvancedGroupsDependingOn(category, group);
                            for (const advGroup of advancedDependentGroups) {
                                console.log(`   🔄 Prepočítavam nadstavbovú skupinu (závisí na ${group}): ${category} - ${advGroup}`);
                ...
                for (const groupKey of affectedGroups) {
                    ...
                    if (isAdvancedGroup) {
                        ...
                        if (processedGroupsInitial.has(baseGroupKey)) {
                            console.log(`   🔄 Prepočítavam nadstavbovú skupinu: ${category} - ${group}`);
                            createAdvancedGroupTable(category, group, baseGroupName);
                        }
                    }
                }
...
function clearAllTeamNameCache() {
    console.log('🗑️ VYMAZÁVAM CACHE názvov tímov...');
    
    // 🔥 POUŽIJEME GLOBÁLNY PRÍSTUP K CACHE
    if (window.__internalReplacementCache) {
        window.__internalReplacementCache.clear();
        console.log('   ✅ replacementCache vymazaná cez window.__internalReplacementCache');
    }
    
    // Pre istotu aj localStorage
    try {
        localStorage.removeItem('teamNameReplacer_cache');
        console.log('   ✅ localStorage cache vymazaná');
    } catch (error) {}
    
    if (window.__teamNameMapping) {
        window.__teamNameMapping = {};
        console.log('   ✅ Globálne mapovanie vymazané');
    }
    
    console.log('✅ Všetky cache boli úspešne vymazané');
}
...
function startGroupMonitoring(intervalSeconds = 5) {
    ...
    groupMonitorInterval = setInterval(() => {
        ...
        if (groupsWithLoss && groupsWithLoss.length > 0) {
            ...
            // Obnovíme stránku
            isReloading = true;
            console.log('🔄 Obnovujem stránku kvôli zmene stavu skupiny (cache bola vymazaná)...');
...
function resetCarryOverCache() {
    processedCarryOverGroups.clear();
    console.log('🗑️ Cache prenášania výsledkov bola vymazaná');
}
...
window.startTeamNameReplacement = async function() {
    ...
    let checkInterval = setInterval(() => {
        if (window.matchTracker && typeof window.matchTracker.getAllMatches === 'function') {
            clearInterval(checkInterval);
            console.log('✅ MatchTracker je pripravený, spúšťam monitorovanie skupín...');
