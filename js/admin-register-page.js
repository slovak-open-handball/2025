...
function AdminRegisterPage() {
  ...
  // Redirect if logged in as admin or if no admin exists (to prevent non-admin from creating)
  React.useEffect(() => {
    if (isAuthReady && isRoleLoaded) {
      ...
      // If no user and admin already exists, redirect to login
      if (!user && adminExists && isRoleLoaded) {
        setError('Administrátor už existuje. Prihláste sa prosím.');
        setTimeout(() => window.location.href = 'login.html', 1500);
      }
    }
  }, [user, isAdmin, isAuthReady, isRoleLoaded, adminExists]);
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">Registrácia Administrátora</h2>
        {!adminExists ? (
          <p className="text-green-600 text-center mb-4">Žiadny administrátor neexistuje. Môžete vytvoriť prvého administrátora.</p>
        ) : (
          <p className="text-red-600 text-center mb-4">Administrátor už existuje. Túto stránku by ste nemali vidieť, ak nie ste prihlásený ako admin.</p>
        )}

        {adminExists && !isAdmin && <div className="text-red-600 text-center mb-4">Nemáte oprávnenie na prístup.</div>}
