...
const SpiderApp = ({ userProfileData }) => {
    ...
    // Funkcia pre vykreslenie úrovne 1 (semifinále, finále, o 3. miesto)
    const renderLevel1 = (spiderData, userProfileData, generationInProgress, generateSingleMatch, deleteSingleMatch, handleTeamClick, removeTeamAssignment) => {
    ...
    const renderLevel2 = (spiderData, userProfileData, generationInProgress, generateSingleMatch, deleteSingleMatch, handleTeamClick, removeTeamAssignment) => {
        return React.createElement(
            React.Fragment,
            null,
    ...
    const renderLevel3 = (spiderData, userProfileData, generationInProgress, generateSingleMatch, deleteSingleMatch, handleTeamClick, removeTeamAssignment) => {
        return React.createElement(
            React.Fragment,
            null,
    ...
    const renderLevel4 = (spiderData, userProfileData, generationInProgress, generateSingleMatch, deleteSingleMatch, handleTeamClick, removeTeamAssignment) => {
        return React.createElement(
            React.Fragment,
            null,
    ...
    // Funkcia na vymazanie jednotlivého pavúkového zápasu a všetkých podradených zápasov
    const deleteSingleMatch = async (matchId, matchType) => {
        ...
        try {
            ...
            // Funkcia na rekurzívne zmazanie všetkých podradených zápasov
            const deleteChildMatches = async (parentMatchType) => {
                let childMatchesToDelete = [];
                
                // Ak mažeme finále, mažeme všetky pavúkové zápasy v kategórii
                if (parentMatchType === 'finále') {
                    childMatchesToDelete = allMatches.filter(m => 
                        m.categoryId === selectedCategory && 
                        m.matchType && 
                        ['semifinále 1', 'semifinále 2', 'o 3. miesto', 
                         'štvrťfinále 1', 'štvrťfinále 2', 'štvrťfinále 3', 'štvrťfinále 4',
                         'osemfinále 1', 'osemfinále 2', 'osemfinále 3', 'osemfinále 4',
                         'osemfinále 5', 'osemfinále 6', 'osemfinále 7', 'osemfinále 8',
                         'šestnásťfinále 1', 'šestnásťfinále 2', 'šestnásťfinále 3', 'šestnásťfinále 4',
                ...
                // Ak mažeme semifinále 1, mažeme všetky zápasy v jeho vetve
                else if (parentMatchType === 'semifinále 1') {
                    childMatchesToDelete = allMatches.filter(m => 
                        m.categoryId === selectedCategory && 
                        m.matchType && 
                        ['štvrťfinále 1', 'štvrťfinále 2', 
                         'osemfinále 1', 'osemfinále 2', 'osemfinále 3', 'osemfinále 4',
                         'šestnásťfinále 1', 'šestnásťfinále 2', 'šestnásťfinále 3', 'šestnásťfinále 4',
                         'šestnásťfinále 5', 'šestnásťfinále 6', 'šestnásťfinále 7', 'šestnásťfinále 8'].includes(m.matchType)
                    );
                }
                // Ak mažeme semifinále 2, mažeme všetky zápasy v jeho vetve
                else if (parentMatchType === 'semifinále 2') {
                    childMatchesToDelete = allMatches.filter(m => 
                        m.categoryId === selectedCategory && 
                        m.matchType && 
                        ['štvrťfinále 3', 'štvrťfinále 4',
                         'osemfinále 5', 'osemfinále 6', 'osemfinále 7', 'osemfinále 8',
                         'šestnásťfinále 9', 'šestnásťfinále 10', 'šestnásťfinále 11', 'šestnásťfinále 12',
                         'šestnásťfinále 13', 'šestnásťfinále 14', 'šestnásťfinále 15', 'šestnásťfinále 16'].includes(m.matchType)
                    );
                ...
                // Ak mažeme štvrťfinále, mažeme všetky zápasy pod ním
                    ...
                    const qfNumber = parseInt(parentMatchType.split(' ')[1]);
                    
                    let eightfinalNumbers = [];
                    let sixteenfinalNumbers = [];
                    
                    if (qfNumber === 1) {
                        eightfinalNumbers = [1, 2];
                        sixteenfinalNumbers = [1, 2, 3, 4];
                    } else if (qfNumber === 2) {
                        eightfinalNumbers = [3, 4];
                        sixteenfinalNumbers = [5, 6, 7, 8];
                    } else if (qfNumber === 3) {
                        eightfinalNumbers = [5, 6];
                        sixteenfinalNumbers = [9, 10, 11, 12];
                    } else if (qfNumber === 4) {
                        eightfinalNumbers = [7, 8];
                        sixteenfinalNumbers = [13, 14, 15, 16];
                    }
                    
                    childMatchesToDelete = allMatches.filter(m => 
                        m.categoryId === selectedCategory && 
                        m.matchType && (
                            eightfinalNumbers.some(num => m.matchType === `osemfinále ${num}`) ||
                            sixteenfinalNumbers.some(num => m.matchType === `šestnásťfinále ${num}`)
                        )
                    );
                }
                // Ak mažeme osemfinále, mažeme všetky zápasy pod ním
                else if (parentMatchType.startsWith('osemfinále')) {
                    const efNumber = parseInt(parentMatchType.split(' ')[1]);
                    
                    let sixteenfinalNumbers = [];
                    if (efNumber === 1) sixteenfinalNumbers = [1, 2];
                    else if (efNumber === 2) sixteenfinalNumbers = [3, 4];
                    else if (efNumber === 3) sixteenfinalNumbers = [5, 6];
                    else if (efNumber === 4) sixteenfinalNumbers = [7, 8];
                    else if (efNumber === 5) sixteenfinalNumbers = [9, 10];
                    else if (efNumber === 6) sixteenfinalNumbers = [11, 12];
                    else if (efNumber === 7) sixteenfinalNumbers = [13, 14];
                    else if (efNumber === 8) sixteenfinalNumbers = [15, 16];
                ...
                return childMatchesToDelete;
            };
    
            // Získame všetky podradené zápasy, ktoré treba zmazať
            const childMatches = await deleteChildMatches(matchType);
            
            // Vytvoríme pole všetkých zápasov na zmazanie (vrátane aktuálneho)
            const allMatchesToDelete = [matchToDelete, ...childMatches];
            
            // Odstránime duplicity (ak nejaký zápas patrí do viacerých kategórií)
            const uniqueMatchesToDelete = allMatchesToDelete.filter((match, index, self) => 
            ...
            for (const match of uniqueMatchesToDelete) {
                ...
                if (match.matchType.startsWith('šestnásťfinále')) {
                    const matchNumber = parseInt(match.matchType.split(' ')[1]);
                    ...
                    if (eightfinalMatch && !uniqueMatchesToDelete.some(m => m.id === eightfinalMatch.id)) {
                        const updateData = {};
                        if (position === 'home') {
                ...
                // Ak mažeme osemfinálový zápas, aktualizujeme príslušný štvrťfinálový zápas
                    ...
                    if (quarterfinalMatch && !uniqueMatchesToDelete.some(m => m.id === quarterfinalMatch.id)) {
                        const updateData = {};
                        if (position === 'home') {
                            updateData.homeTeamIdentifier = '---';
                        } else {
                ...
                // Ak mažeme štvrťfinálový zápas, aktualizujeme príslušný semifinálový zápas
                else if (match.matchType.startsWith('štvrťfinále')) {
                    const matchNumber = parseInt(match.matchType.split(' ')[1]);
                    ...
                    if (semifinalMatch && !uniqueMatchesToDelete.some(m => m.id === semifinalMatch.id)) {
                        const updateData = {};
                        if (position === 'home') {
                            updateData.homeTeamIdentifier = '---';
                        } else {
                ...
                // Ak mažeme semifinálový zápas, aktualizujeme finále a o 3. miesto
                else if (match.matchType.startsWith('semifinále')) {
                    const matchNumber = parseInt(match.matchType.split(' ')[1]);
                    ...
                    if (finalMatch && !uniqueMatchesToDelete.some(m => m.id === finalMatch.id)) {
                        if (matchNumber === 1) {
                    ...
                    if (thirdPlaceMatch && !uniqueMatchesToDelete.some(m => m.id === thirdPlaceMatch.id)) {
                        if (matchNumber === 1) {
            ...
            // Zostavíme správu o počte zmazaných zápasov
            let message;
            if (uniqueMatchesToDelete.length === 1) {
                message = `Zápas ${matchType} bol zmazaný`;
            } else {
                message = `Zmazaných ${uniqueMatchesToDelete.length} zápasov (${matchType} a všetky podradené zápasy)`;
            }
    ...
    const MatchCell = ({ match, title = '', matchType, userProfileData, generationInProgress, onGenerate, onDelete, onTeamClick, onRemoveTeam, isFilterActive, allMatches }) => {
            ...
            return React.createElement(
                ...
                { 
                    className: `border-2 border-dashed border-gray-400 rounded-lg p-3 min-w-[200px] transition-all duration-200 ${
                        isHovered && userProfileData?.role === 'admin' && !generationInProgress && isFilterActive
                    ...
                    style: { 
                        ...
                        cursor: userProfileData?.role === 'admin' && isFilterActive ? 'pointer' : 'default'
                    },
                    onMouseEnter: () => {
                        if (userProfileData?.role === 'admin' && !generationInProgress && isFilterActive) {
                            setIsHovered(true);
                        }
                    },
                    onMouseLeave: () => {
        ...
        return React.createElement(
            ...
            React.createElement(
                'div',
                { 
                    className: `border-2 border-gray-300 rounded-lg p-3 min-w-[200px] bg-white shadow-sm group relative ${isMatchCompleted ? 'opacity-75' : ''}`,
                ...
                // Ikona koša pre adminov pre celý zápas - NEBUDE SA ZOBRAZOVAŤ PRE UKONČENÝ ZÁPAS
                userProfileData?.role === 'admin' && match.exists && isFilterActive && !isMatchCompleted && React.createElement(
                    'div',
                    { 
                ...
                // Domáci tím - s ikonami pre úpravu a odstránenie (NEBUDE SA ZOBRAZOVAŤ PRE UKONČENÝ ZÁPAS)
                React.createElement(
                    'div',
                    { 
                    ...
                    // Ikony pre adminov - NEBUDÚ SA ZOBRAZOVAŤ PRE UKONČENÝ ZÁPAS
                    userProfileData?.role === 'admin' && isFilterActive && !isMatchCompleted && React.createElement(
                        'div',
                        { 
                ...
                // Hosťovský tím - s ikonami pre úpravu a odstránenie (NEBUDE SA ZOBRAZOVAŤ PRE UKONČENÝ ZÁPAS)
                React.createElement(
                    'div',
                    { 
                        className: `flex justify-between items-center py-2 border-b border-gray-100 group/team`,
                ...
                (formattedDate || matchTime || hallName) && React.createElement(
                    ...
                    matchTime && React.createElement(
                        ...
                        React.createElement('span', null, matchTime + ' hod.')
                    ),
        ...
        React.createElement(
            ...
            !selectedCategory ? (
                ...
                React.createElement(
                    ...
                    // Ak nemáme žiadne kategórie s pavúkmi
                    (() => {
                        // Získanie všetkých kategórií, ktoré majú pavúkové zápasy
                        const categoriesWithSpider = categories.filter(category => {
                            const spiderMatches = allMatches.filter(m => 
                        ...
                        if (sortedCategoriesWithSpider.length === 0) {
                            return React.createElement(
                                ...
                                React.createElement('p', { className: 'text-lg' }, 'Vyberte kategóriu pre zobrazenie pavúka alebo vytvorte nový pavúk kliknutím na zelenú polovicu tlačidla "+"')
                            );
                        }
                        ...
                        return sortedCategoriesWithSpider.map(category => {
                            ...
                            const hasSixteenfinals = categorySpiderMatches.some(m => m.matchType.startsWith('šestnásťfinále'));
                            const hasEightfinals = categorySpiderMatches.some(m => m.matchType.startsWith('osemfinále'));
                            const hasQuarterfinals = categorySpiderMatches.some(m => m.matchType.startsWith('štvrťfinále'));
                            
                            let categorySpiderLevel = 1;
                            if (hasSixteenfinals) categorySpiderLevel = 4;
                            else if (hasEightfinals) categorySpiderLevel = 3;
                            else if (hasQuarterfinals) categorySpiderLevel = 2;
                            
                            // Vytvorenie štruktúry pre túto kategóriu
                            const categorySpiderStructure = {
                                final: categorySpiderMatches.find(m => m.matchType === 'finále') || { exists: false },
                                semiFinals: [
                                    categorySpiderMatches.find(m => m.matchType === 'semifinále 1') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'semifinále 2') || { exists: false }
                                ],
                                quarterFinals: [
                                    categorySpiderMatches.find(m => m.matchType === 'štvrťfinále 1') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'štvrťfinále 2') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'štvrťfinále 3') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'štvrťfinále 4') || { exists: false }
                                ],
                                eightFinals: [
                                    categorySpiderMatches.find(m => m.matchType === 'osemfinále 1') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'osemfinále 2') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'osemfinále 3') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'osemfinále 4') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'osemfinále 5') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'osemfinále 6') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'osemfinále 7') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'osemfinále 8') || { exists: false }
                                ],
                                sixteenFinals: [
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 1') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 2') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 3') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 4') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 5') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 6') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 7') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 8') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 9') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 10') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 11') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 12') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 13') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 14') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 15') || { exists: false },
                                    categorySpiderMatches.find(m => m.matchType === 'šestnásťfinále 16') || { exists: false }
                                ],
                                thirdPlace: categorySpiderMatches.find(m => m.matchType === 'o 3. miesto') || { exists: false }
                            };
                            
                            // Naplnenie dátami
                            categorySpiderMatches.forEach(match => {
                                if (match.matchType === 'finále') {
                                    categorySpiderStructure.final = { ...categorySpiderStructure.final, ...match, exists: true };
                                } else if (match.matchType === 'semifinále 1') {
                                    categorySpiderStructure.semiFinals[0] = { ...categorySpiderStructure.semiFinals[0], ...match, exists: true };
                                } else if (match.matchType === 'semifinále 2') {
                                    categorySpiderStructure.semiFinals[1] = { ...categorySpiderStructure.semiFinals[1], ...match, exists: true };
                                } else if (match.matchType === 'o 3. miesto') {
                                    categorySpiderStructure.thirdPlace = { ...categorySpiderStructure.thirdPlace, ...match, exists: true };
                                } else if (match.matchType === 'štvrťfinále 1') {
                                    categorySpiderStructure.quarterFinals[0] = { ...categorySpiderStructure.quarterFinals[0], ...match, exists: true };
                                } else if (match.matchType === 'štvrťfinále 2') {
                                    categorySpiderStructure.quarterFinals[1] = { ...categorySpiderStructure.quarterFinals[1], ...match, exists: true };
                                } else if (match.matchType === 'štvrťfinále 3') {
                                    categorySpiderStructure.quarterFinals[2] = { ...categorySpiderStructure.quarterFinals[2], ...match, exists: true };
                                } else if (match.matchType === 'štvrťfinále 4') {
                                    categorySpiderStructure.quarterFinals[3] = { ...categorySpiderStructure.quarterFinals[3], ...match, exists: true };
                                } else if (match.matchType.startsWith('osemfinále')) {
                                    const index = parseInt(match.matchType.split(' ')[1]) - 1;
                                    if (index >= 0 && index < 8) {
                                        categorySpiderStructure.eightFinals[index] = { ...categorySpiderStructure.eightFinals[index], ...match, exists: true };
                                    }
                                } else if (match.matchType.startsWith('šestnásťfinále')) {
                                    const index = parseInt(match.matchType.split(' ')[1]) - 1;
                                    if (index >= 0 && index < 16) {
                                        categorySpiderStructure.sixteenFinals[index] = { ...categorySpiderStructure.sixteenFinals[index], ...match, exists: true };
                                    }
                                }
                            });
                            
                            // Výber správnej renderovacej funkcie podľa úrovne
                            let renderFunction;
                            if (categorySpiderLevel === 4) renderFunction = renderLevel4;
                            else if (categorySpiderLevel === 3) renderFunction = renderLevel3;
                            else if (categorySpiderLevel === 2) renderFunction = renderLevel2;
                            ...
                            return React.createElement(
                                ...
                                { 
                                    key: category.id,
                                    className: 'border border-gray-200 rounded-xl p-6 bg-gray-50 w-full',
                                    style: { boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }
                                },
                                React.createElement(
                                    'h2',
                                    { 
                                        className: 'text-2xl font-bold mb-6 text-gray-800 text-center border-b pb-3',
                                    },
                                    `${category.name} ${categorySpiderLevel === 4 ? '(šestnásťfinále)' : (categorySpiderLevel === 3 ? '(osemfinále)' : (categorySpiderLevel === 2 ? '(štvrťfinále)' : ''))}`
                                ),
                                React.createElement(
                                    'div',
                                    { 
                                        ...
                                        style: { 
                                            width: '100%',
                                            maxWidth: categorySpiderLevel === 4 ? `${7 * (220 + 24 + 4)}px` : 
                                                        categorySpiderLevel === 3 ? `${5 * (220 + 24 + 4)}px` : 
                                                          (categorySpiderLevel === 2 ? `${3 * (240 + 24 + 4)}px` : 
                                                                            `${2 * (260 + 24 + 4)}px`),
                                            margin: '0 auto'
                                        }
                                    },
                ...
                React.createElement(
                    ...
                    // ROVNAKÝ BOX AKO PRI NEVYFILTROVANEJ KATEGÓRII
                    React.createElement(
                        'div',
                        { 
                            className: 'border border-gray-200 rounded-xl p-6 bg-gray-50 w-full',
                        ...
                        React.createElement(
                            ...
                            `${categories.find(c => c.id === selectedCategory)?.name || selectedCategory} ${spiderLevel === 4 ? '(šestnásťfinále)' : (spiderLevel === 3 ? '(osemfinále)' : (spiderLevel === 2 ? '(štvrťfinále)' : ''))}`
                        ),
                        
                        // Tabuľka pre pavúka - ROVNAKÁ ŠTRUKTÚRA AKO PRI NEVYFILTROVANÝCH PAVÚKOCH
                        React.createElement(
                            'div',
                            { 
                                className: 'overflow-x-auto',
