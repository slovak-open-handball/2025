...
const AssignMatchModal = ({ isOpen, onClose, match, sportHalls, categories, onAssign, allMatches, displayMode, getTeamDisplayText, initialFilters, blockedBreaks, groupsByCategory = {} }) => {
    ...
    const calculateFirstAvailableTime = (hallId, date, existingMatchesList, hallStartTimeStr, matchDur, blockedBreaks, allMatches, relatedMatchesList, isAdvanced, currentMatchId) => {
        if (!hallId || !date || !hallStartTimeStr || matchDur === 0) {
            return null;
        }
        ...
        const allMatchesForHallAndDay = allMatches.filter(m => 
            m.hallId === hallId && 
            m.scheduledTime &&
            m.id !== currentMatchId  // ← POUŽIJTE currentMatchId namiesto match?.id
        ).filter(m => {
        ...
        // PRIDÁME VŠETKY ZÁPASY (BEZ OHLADU NA SKUPINU)
        ...
        // 🔥 PRIDÁME SÚVISIACE ZÁPASY (NADSTAVBOVÁ SKUPINA) - ak je isAdvanced true
        if (isAdvanced && relatedMatchesList && relatedMatchesList.length > 0) {
            const relatedMatchesForDay = relatedMatchesList.filter(relatedMatch => {
    ...
    useEffect(() => {
        const loadHallStartTime = async () => {
            if (selectedHallId && selectedDate && window.db) {
                ...
                try {
                    ...
                    // 🔥 VÝPOČET NAVRHOVANÉHO ČASU - používa VŠETKY zápasy, nie len relatedMatches
        ...
        loadHallStartTime();
    }, [selectedHallId, selectedDate, matchDuration, categoryDetails, existingMatches, selectedTime, allMatches, blockedBreaks, relatedMatches, isAdvancedGroup, match?.id]);
    ...
    useEffect(() => {
        if (selectedTime && matchDuration > 0) {
            ...
            if (isAdvancedGroup && relatedMatches.length > 0) {
                ...
                // 🔥 1. KONTROLA ČASOVÉHO PREKRÝVANIA (v rovnaký deň)
    ...
    const canSave = selectedHallId && selectedDate && selectedTime && !hasError && match?.id;
